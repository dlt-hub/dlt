...
def always_choose(
    ...
    Args:
        ...
        always_confirm: When True, confirm calls always return True, regardless of `always_choose_default` and `always_choose_value`.
    """
...
def maybe_no_stdin() -> ContextManager[None]:
    ...
    return always_choose(
        True if not sys.stdin.isatty() else ALWAYS_CHOOSE_DEFAULT, ALWAYS_CHOOSE_VALUE, ALWAYS_CONFIRM
    )
