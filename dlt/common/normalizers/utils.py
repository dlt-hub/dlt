...
from dlt.common.normalizers.json import DataItemNormalizer, SupportsDataItemNormalizer
...
def import_normalizers(
...
) -> t.Tuple[NamingConvention, t.Type[DataItemNormalizer[t.Any]]]:
    ...
    try:
        if "." in names:
            ...
            naming_module = t.cast(SupportsNamingConvention, import_module(names))
        else:
            # from known location
            naming_module = t.cast(
    ...
    if destination_capabilities:
        ...
        max_length = None
    json_module = t.cast(
        SupportsDataItemNormalizer, import_module(normalizers_config["json"]["module"])
    )
