...
def get_literal_defaults(node: ast.FunctionDef) -> t.Dict[str, str]:
    ...
    literal_defaults: t.Dict[str, str] = {}
...
def find_outer_func_def(node: ast.AST) -> t.Optional[ast.FunctionDef]:
...
def creates_func_def_name_node(
    func_def: ast.FunctionDef, source_lines: t.Sequence[str]
) -> ast.Name:
...
def rewrite_python_script(
    source_script_lines: t.List[str], transformed_nodes: t.List[t.Tuple[ast.AST, ast.AST]]
) -> t.List[str]:
...
def evaluate_node_literal(node: ast.AST) -> t.Any:
    try:
