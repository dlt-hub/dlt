...
class LoadStorage(VersionedStorage):
    ...
    def create_temp_load_folder(self, load_id: str) -> None:
        ...
        self.storage.create_folder(f"{load_id}/{LoadStorage.NEW_JOBS_FOLDER}")
        self.storage.create_folder(f"{load_id}/{LoadStorage.COMPLETED_JOBS_FOLDER}")
        self.storage.create_folder(f"{load_id}/{LoadStorage.FAILED_JOBS_FOLDER}")
        self.storage.create_folder(f"{load_id}/{LoadStorage.STARTED_JOBS_FOLDER}")
    ...
    def load_schema(self, load_id: str) -> Schema:
        ...
        schema_path = f"{self.get_load_path(load_id)}/{LoadStorage.SCHEMA_FILE_NAME}"
    ...
    def load_temp_schema(self, load_id: str) -> Schema:
        ...
        schema_path = f"{load_id}/{LoadStorage.SCHEMA_FILE_NAME}"
        return self._load_schema(schema_path)
    ...
    def save_temp_schema(self, schema: Schema, load_id: str) -> str:
        ...
        return self.storage.save(f"{load_id}/{LoadStorage.SCHEMA_FILE_NAME}", dump)

    def save_temp_schema_updates(self, load_id: str, schema_updates: Sequence[TSchemaUpdate]) -> None:
        with self.storage.open_file(f"{load_id}/{LoadStorage.LOAD_SCHEMA_UPDATE_FILE_NAME}", mode="w") as f:
    ...
    def list_new_jobs(self, load_id: str) -> Sequence[str]:
        new_jobs = self.storage.list_folder_files(f"{self.get_load_path(load_id)}/{LoadStorage.NEW_JOBS_FOLDER}")
    ...
    def list_started_jobs(self, load_id: str) -> Sequence[str]:
        return self.storage.list_folder_files(f"{self.get_load_path(load_id)}/{LoadStorage.STARTED_JOBS_FOLDER}")

    def list_failed_jobs(self, load_id: str) -> Sequence[str]:
        return self.storage.list_folder_files(f"{self.get_load_path(load_id)}/{LoadStorage.FAILED_JOBS_FOLDER}")

    def list_archived_failed_jobs(self, load_id: str) -> Sequence[str]:
        return self.storage.list_folder_files(f"{self.get_archived_path(load_id)}/{LoadStorage.FAILED_JOBS_FOLDER}")

    def begin_schema_update(self, load_id: str) -> Optional[TSchemaUpdate]:
        schema_update_file = f"{self.get_load_path(load_id)}/{LoadStorage.LOAD_SCHEMA_UPDATE_FILE_NAME}"
    ...
    def commit_schema_update(self, load_id: str) -> None:
        ...
        schema_update_file = f"{load_path}/{LoadStorage.LOAD_SCHEMA_UPDATE_FILE_NAME}"
        self.storage.atomic_rename(schema_update_file, f"{load_path}/{LoadStorage.COMPLETED_JOBS_FOLDER}/{LoadStorage.LOAD_SCHEMA_UPDATE_FILE_NAME}")
    ...
    def fail_job(self, load_id: str, file_name: str, failed_message: Optional[str]) -> str:
        load_path = self.get_load_path(load_id)
        if failed_message:
            self.storage.save(f"{load_path}/{LoadStorage.FAILED_JOBS_FOLDER}/{file_name}.exception", failed_message)
        # move to failed jobs
    ...
    def get_load_path(self, load_id: str) -> str:
        return f"{LoadStorage.NORMALIZED_FOLDER}/{load_id}"

    def get_archived_path(self, load_id: str) -> str:
        return f"{LoadStorage.LOADED_FOLDER}/{load_id}"
    ...
    def build_loading_file_name(self, load_id: str, table_name: str, file_id: str) -> str:
        ...
        return f"{load_id}/{LoadStorage.NEW_JOBS_FOLDER}/{file_name}"
