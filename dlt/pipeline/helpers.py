...
class pipeline_drop:
    def __init__(
        ...
        collected during the dry run by inspecting the `info` property.
        ...
        drop_result = prepare_drop_resources(
...
def prepare_refresh_source(
    ...
    """Prepare refresh mode on the given source, updating the provided `schema` and pipeline state
       to reflect tables being dropped/truncated and associated resources state.

       Behavior depends in `refresh` mode:
       * `drop_sources` - all tables generated by resources in `source` will be dropped, source state will be wiped.
       * `drop_resources` - tables generated by resources that are actually selected in `source` will be dropped and associated resources stare will be wiped.
       * `drop_data` - tables generated by resources that are actually selected in `source` will be truncated and associated resources stare will be wiped.
    ...
    drop_result = prepare_drop_resources(
        # do not clone the schema, change in place
        source.schema,
...
class pipeline_abort:
    def __init__(self, pipeline: "Pipeline", load_ids: Sequence[str] = ()) -> None:
        ...
        by inspecting the `info` property before calling. The abort operation performs the following:
