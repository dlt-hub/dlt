...
# TODO rename `allow_unknown_columns` to `infer_sqlglot_schema`;  this would match the SQLGlot `infer_schema` kwarg;
# even if `allow_unknown_columns=True`, some queries haved undetermined final columns
# TODO rename `allow_fail` to `allow_partial` or `allow_incomplete`. This would ensure a dictionary is always returned
# even in cases of SQL parsing errors, missing table reference, unresolved `SELECT *`
def compute_columns_schema(
    ...
    try:
        ...
        if allow_fail:
            logger.debug("Failed to parse the SQL query. Returning empty table schema because `allow_fail=True`")
            return {}
    ...
    if not isinstance(expression, sge.Select):
        if allow_fail:
            logger.debug("Parsed SQL query is not a SELECT statement. Returning empty table schema because `allow_fail=True`")
            return {}

        raise LineageFailedException(
            f"Parsed SQL query is not a SELECT statement. Received SQL expression of type {expression.type}."
        )
    ...
    if allow_anonymous_columns is False:
       for col in expression.selects:
           if col.output_name == "":
               raise LineageFailedException(
                   "Found anonymous column in SELECT statement. Use `allow_anonymous_columns=True` for permissive handling.\n"
                   f"Column:\n\t{col}"
               )
    ...
    try:
        expression = qualify(
            ...
            dialect=dialect,
            infer_schema=allow_unknown_columns,
        )
    ...
    for col in expression.selects:
        if col.output_name == "*":
            ...
            raise LineageFailedException(
                "SELECT statement includes a `*` selection that can't be resolved. "
                "Modify the query to select columns explicitly or limit `*` to known tables (e.g., `SELECT known_table.*`). "
                "Use `allow_fail=True` to return a partial dlt schema with resolvable column hints.\n"
                f"Column:\n\t{col}"
            )
