...
def _create_dbt_deps(destination_types: List[str], dbt_version: str = DEFAULT_DBT_VERSION) -> List[str]:
    if dbt_version:
...
def restore_venv(venv_dir: str, destination_types: List[str], dbt_version: str = DEFAULT_DBT_VERSION) -> Venv:
    venv = Venv.restore(venv_dir)
    venv.add_dependencies(_create_dbt_deps(destination_types, dbt_version))
    return venv


def create_venv(venv_dir: str, destination_types: List[str], dbt_version: str = DEFAULT_DBT_VERSION) -> Venv:
