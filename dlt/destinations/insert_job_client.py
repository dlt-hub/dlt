...
class InsertValuesLoadJob(LoadJob, FollowupJob):
    def __init__(self, table_name: str, use_staging_table: bool, truncate_destination_table: bool, file_path: str, sql_client: SqlClientBase[Any]) -> None:
        super().__init__(FileStorage.get_file_name_from_file_path(file_path))
        self._sql_client = sql_client
        # insert file content immediately
        with self._sql_client.with_staging_dataset(use_staging_table):
            with self._sql_client.begin_transaction():
                for fragments in self._insert(sql_client.make_qualified_table_name(table_name), truncate_destination_table, file_path):
    ...
    def _insert(self, qualified_table_name: str, truncate_destination_table: bool, file_path: str) -> Iterator[List[str]]:
...
class InsertValuesJobClient(SqlJobClientBase):
    ...
    def start_file_load(self, table: TTableSchema, file_path: str, load_id: str) -> LoadJob:
        ...
        if not job:
            ...
            job = InsertValuesLoadJob(table["name"], disposition in self.get_stage_dispositions(), self.truncate_destination_table(disposition), file_path, self.sql_client)
        return job
