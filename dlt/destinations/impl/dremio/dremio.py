...
class DLTDremioFormatter(DefaultParameterFormatter):
    _INSTANCE: ClassVar["DLTDremioFormatter"] = None

    def __new__(cls: Type["DLTDremioFormatter"]) -> "DLTDremioFormatter":
        if cls._INSTANCE:
            return cls._INSTANCE
        return super().__new__(cls)

    def __init__(self) -> None:
        if DLTDremioFormatter._INSTANCE:
            return
        ...
        DLTDremioFormatter._INSTANCE = self
...
class DremioSQLClient(SqlClientBase[Connection]):
    capabilities: ClassVar[DestinationCapabilitiesContext] = capabilities()
    dbapi: ClassVar[DBApi] = pyathena

    def __init__(self, dataset_name: str, config: DremioClientConfiguration) -> None:
    ...
    def escape_ddl_identifier(self, v: str) -> str:
        ...
        # Dremio uses HIVE to create tables but for querying it uses PRESTO (so normal escaping)
        if not v:
            return v
    ...
    def execute_query(self, query: AnyStr, *args: Any, **kwargs: Any) -> Iterator[DBApiCursor]:
        ...
        cursor = self._conn.cursor(formatter=DLTDremioFormatter())
...
class DremioClient(SqlJobClientWithStaging, SupportsStagingDestination):
    ...
    def __init__(self, schema: Schema, config: DremioClientConfiguration) -> None:
        # verify if staging layout is valid for Dremio
        ...
        sql_client = DremioSQLClient(config.normalize_dataset_name(schema), config)
    ...
    def start_file_load(self, table: TTableSchema, file_path: str, load_id: str) -> LoadJob:
        ...
        if table_schema_has_type(table, "time"):
            raise LoadJobTerminalException(
                file_path,
                "Dremio cannot load TIME columns from parquet tables. Please convert"
