...
class LanceDBClientOptions(BaseConfiguration):
    storage_options: Optional[DictStrStr] = dataclasses.field(
        default_factory=_default_storage_options
    )
    """Dictionary containing configuration options for connecting to different storage backends.

    The `storage_options` dictionary allows you to specify various settings required for connecting to storage services such as AWS S3,
    Google Cloud Storage, and Azure Blob Storage. These settings can include parameters like `region`, `endpoint`, and other service-specific configurations.
    ...
    You can find more information about the available options and their usage in the LanceDB storage configuration guide:
...
class LanceDBClientConfiguration(DestinationClientDwhConfiguration):
    ...
    embedding_provider: TEmbeddingProvider = "cohere"
    """Embedding provider used for generating embeddings. Default is "cohere".
    You can find the full list of providers at https://github.com/lancedb/lancedb/tree/main/python/python/lancedb/embeddings."""
