...
class LoadLanceDBJob(LoadJob):
    ...
    def _upload_data(
        ...
        """Uploads data to a LanceDB instance in a batch. Supports retries and parallelism.
    ...
    def _generate_uuid(
        ...
        Args:
            ...
            collection_name (str): LanceDB collection name.
...
class LanceDBClient(JobClientBase, WithStateSync):
    ...
    def __init__(self, schema: Schema, config: LanceDBClientConfiguration) -> None:
        ...
        self.db_client: QC = LanceDBClient._create_db_client(config)
    ...
    def _create_db_client(config: LanceDBClientConfiguration) -> QC:
        """Generates a LanceDB client from the 'LanceDB_client' package.

        Args:
            config (LanceDBClientConfiguration): Credentials and options for the LanceDB client.

        Returns:
            LanceDBClient: A LanceDB client instance.
        """
    ...
    def _create_point(self, obj: Dict[str, Any], collection_name: str) -> None:
        """Inserts a point into a LanceDB collection without a vector.
    ...
    def _update_schema_in_storage(self, schema: Schema) -> None:
        schema_def = json.dumps(schema.to_dict())
        properties = {
