...
from typing import (List, Any, cast, Union, Tuple, Iterable, Type, Optional, Dict, Sequence, TYPE_CHECKING, Set, )
...
from dlt.common.destination.exceptions import (DestinationUndefinedEntity, DestinationTransientException,
                                               DestinationTerminalException, )
from dlt.common.destination.reference import (JobClientBase, WithStateSync, LoadJob, StorageSchemaInfo, StateInfo,
                                              TLoadJobState, NewLoadJob, FollowupJob, )
from dlt.common.pendulum import timedelta
from dlt.common.schema import Schema, TTableSchema, TSchemaTables
from dlt.common.schema.typing import (TColumnType, TTableFormat, TTableSchemaColumns, TWriteDisposition, )
...
from dlt.destinations.impl.lancedb.lancedb_adapter import (VECTORIZE_HINT, DOCUMENT_ID_HINT, )
from dlt.destinations.impl.lancedb.schema import (make_arrow_field_schema, make_arrow_table_schema, TArrowSchema,
                                                  NULL_SCHEMA, TArrowField, )
from dlt.destinations.impl.lancedb.utils import (list_merge_identifiers, generate_uuid,
...
class LanceDBTypeMapper(TypeMapper):
    sct_to_unbound_dbt = {"text": pa.string(), "double": pa.float64(), "bool": pa.bool_(), "bigint": pa.int64(), "binary": pa.binary(), "date": pa.date32(), "complex": pa.string(), }

    sct_to_dbt = {}

    dbt_to_sct = {pa.string(): "text", pa.float64(): "double", pa.bool_(): "bool", pa.int64(): "bigint", pa.binary(): "binary", pa.date32(): "date", }

    def to_db_decimal_type(self, precision: Optional[int], scale: Optional[int]) -> pa.Decimal128Type:
        precision, scale = self.decimal_precision(precision, scale)
        return pa.decimal128(precision, scale)

    def to_db_datetime_type(self, precision: Optional[int], table_format: TTableFormat = None) -> pa.TimestampType:
        unit: str = TIMESTAMP_PRECISION_TO_UNIT[self.capabilities.timestamp_precision]
        return pa.timestamp(unit, "UTC")

    def to_db_time_type(self, precision: Optional[int], table_format: TTableFormat = None) -> pa.Time64Type:
        unit: str = TIMESTAMP_PRECISION_TO_UNIT[self.capabilities.timestamp_precision]
        return pa.time64(unit)

    def from_db_type(self, db_type: pa.DataType, precision: Optional[int] = None, scale: Optional[int] = None, ) -> TColumnType:
        if isinstance(db_type, pa.TimestampType):
            return dict(data_type="timestamp", precision=UNIT_TO_TIMESTAMP_PRECISION[db_type.unit], scale=scale, )
        if isinstance(db_type, pa.Time64Type):
            return dict(data_type="time", precision=UNIT_TO_TIMESTAMP_PRECISION[db_type.unit], scale=scale, )
...
def upload_batch(records: List[DictStrAny], /, *, db_client: DBConnection, table_name: str, write_disposition: Optional[TWriteDisposition] = "append", id_field_name: Optional[str] = None, ) -> None:
    ...
    try:
        ...
        raise DestinationTransientException("Couldn't open lancedb database. Batch WILL BE RETRIED") from e
    ...
    try:
        if write_disposition in ("append", "skip"):
            ...
            tbl.merge_insert(id_field_name).when_matched_update_all().when_not_matched_insert_all().execute(records)
        else:
            raise DestinationTerminalException(f"Unsupported write disposition {write_disposition} for LanceDB Destination - batch"
                                               " failed AND WILL **NOT** BE RETRIED.")
    except ArrowInvalid as e:
        raise DestinationTerminalException("Python and Arrow datatype mismatch - batch failed AND WILL **NOT** BE RETRIED.") from e
...
class LanceDBClient(JobClientBase, WithStateSync):
    ...
    def __init__(self, schema: Schema, config: LanceDBClientConfiguration, capabilities: DestinationCapabilitiesContext, ) -> None:
        super().__init__(schema, config, capabilities)
        self.config: LanceDBClientConfiguration = config
        self.db_client: DBConnection = lancedb.connect(uri=self.config.credentials.uri, api_key=self.config.credentials.api_key, read_consistency_interval=timedelta(0), )
        ...
        set_non_standard_providers_environment_variables(embedding_model_provider, self.config.credentials.embedding_model_provider_api_key, )
        ...
        if embedding_model_provider=="openai":
            ...
            self.model_func = PatchedOpenAIEmbeddings(max_retries=self.config.options.max_retries, api_key=self.config.credentials.api_key, )
        else:
            self.model_func = self.registry.get(embedding_model_provider).create(name=self.config.embedding_model, max_retries=self.config.options.max_retries, api_key=self.config.credentials.api_key, )
    ...
    def get_table_schema(self, table_name: str) -> TArrowSchema:
        ...
        return cast(TArrowSchema, schema, )
    ...
    def query_table(self, table_name: str, query: Union[List[Any], NDArray, Array, ChunkedArray, str, Tuple[Any], None] = None, ) -> LanceQueryBuilder:
    ...
    def initialize_storage(self, truncate_tables: Iterable[str] = None) -> None:
        if not self.is_storage_initialized():
            ...
            for table_name in truncate_tables:
                ...
                self.create_table(table_name=fq_table_name, schema=schema, )
    ...
    def get_storage_table(self, table_name: str) -> Tuple[bool, TTableSchemaColumns]:
        ...
        for field in arrow_schema:
            ...
            table_schema[name] = {"name": name, **self.type_mapper.from_db_type(field.type), }
        return True, table_schema

    @lancedb_error
    def add_table_fields(self, table_name: str, field_schemas: List[TArrowField]) -> Optional[Table]:
    ...
    def _execute_schema_update(self, only_tables: Iterable[str]) -> None:
        for table_name in only_tables or self.schema.tables:
            ...
            new_columns = self.schema.get_new_table_columns(table_name, existing_columns, self.capabilities.generates_case_sensitive_identifiers(), )
            logger.info(f"Found {len(new_columns)} updates for {table_name} in {self.schema.name}")
            if len(new_columns) > 0:
                if exists:
                    field_schemas: List[TArrowField] = [make_arrow_field_schema(column["name"], column, self.type_mapper) for column in new_columns]
                    fq_table_name = self.make_qualified_table_name(table_name)
                    self.add_table_fields(fq_table_name, field_schemas)
                else:
                    if table_name not in self.schema.dlt_table_names():
                        embedding_fields = get_columns_names_with_prop(self.schema.get_table(table_name=table_name), VECTORIZE_HINT)
                    ...
                    table_schema: TArrowSchema = make_arrow_table_schema(table_name, schema=self.schema, type_mapper=self.type_mapper, embedding_fields=embedding_fields, embedding_model_func=embedding_model_func, embedding_model_dimensions=embedding_model_dimensions, vector_field_name=vector_field_name, id_field_name=id_field_name, )
                    fq_table_name = self.make_qualified_table_name(table_name)
    ...
    def update_schema_in_storage(self) -> None:
        records = [{self.schema.naming.normalize_identifier("version"): self.schema.version, self.schema.naming.normalize_identifier("engine_version"): self.schema.ENGINE_VERSION,
            self.schema.naming.normalize_identifier("inserted_at"): str(pendulum.now()), self.schema.naming.normalize_identifier("schema_name"): self.schema.name,
            self.schema.naming.normalize_identifier("version_hash"): self.schema.stored_version_hash, self.schema.naming.normalize_identifier("schema"): json.dumps(self.schema.to_dict()), }]
        fq_version_table_name = self.make_qualified_table_name(self.schema.version_table_name)
        write_disposition = self.schema.get_table(self.schema.version_table_name).get("write_disposition")

        upload_batch(records, db_client=self.db_client, table_name=fq_version_table_name, write_disposition=write_disposition, )
    ...
    def get_stored_state(self, pipeline_name: str) -> Optional[StateInfo]:
        ...
        state_table = (state_table_.search().where(f"`{p_pipeline_name}` = '{pipeline_name}'", prefilter=True).to_arrow())
        loads_table = loads_table_.search().where(f"`{p_status}` = 0", prefilter=True).to_arrow()

        # Join arrow tables in-memory.
        joined_table: pa.Table = state_table.join(loads_table, keys=p_dlt_load_id, right_keys=p_load_id, join_type="inner").sort_by([(p_dlt_load_id, "descending")])

        if joined_table.num_rows==0:
            return None

        state = joined_table.take([0]).to_pylist()[0]
        return StateInfo(version=state[p_version], engine_version=state[p_engine_version], pipeline_name=state[p_pipeline_name], state=state[p_state], created_at=pendulum.instance(
            state[p_created_at]), version_hash=state[p_version_hash], _dlt_load_id=state[p_dlt_load_id], )
    ...
    def get_stored_schema_by_hash(self, schema_hash: str) -> Optional[StorageSchemaInfo]:
        ...
        try:
            schemas = (version_table.search().where(f'`{p_version_hash}` = "{schema_hash}"', prefilter=True)).to_list()

            most_recent_schema = sorted(schemas, key=lambda x: x[p_inserted_at], reverse=True)[0]
            return StorageSchemaInfo(version_hash=most_recent_schema[p_version_hash], schema_name=most_recent_schema[p_schema_name], version=most_recent_schema[p_version], engine_version=
            most_recent_schema[p_engine_version], inserted_at=most_recent_schema[p_inserted_at], schema=most_recent_schema[p_schema], )
        except IndexError:
            return None
    ...
    def get_stored_schema(self) -> Optional[StorageSchemaInfo]:
        ...
        try:
            schemas = (version_table.search().where(f'`{p_schema_name}` = "{self.schema.name}"', prefilter=True)).to_list()
    ...
    def complete_load(self, load_id: str) -> None:
        records = [{self.schema.naming.normalize_identifier("load_id"): load_id, self.schema.naming.normalize_identifier("schema_name"): self.schema.name,
            self.schema.naming.normalize_identifier("status"): 0, self.schema.naming.normalize_identifier("inserted_at"): str(pendulum.now()),
            self.schema.naming.normalize_identifier("schema_version_hash"): None,  # Payload schema must match the target schema.
        }]
        fq_loads_table_name = self.make_qualified_table_name(self.schema.loads_table_name)
        write_disposition = self.schema.get_table(self.schema.loads_table_name).get("write_disposition")
        upload_batch(records, db_client=self.db_client, table_name=fq_loads_table_name, write_disposition=write_disposition, )
    ...
    def start_file_load(self, table: TTableSchema, file_path: str, load_id: str) -> LoadJob:
        ...
        return LoadLanceDBJob(self.schema, table, file_path, type_mapper=self.type_mapper, db_client=self.db_client, client_config=self.config, model_func=self.model_func, fq_table_name=self.make_qualified_table_name(
            table["name"]), fq_parent_table_name=(self.make_qualified_table_name(parent_table) if parent_table else None), )

    def create_table_chain_completed_followup_jobs(self, table_chain: Sequence[TTableSchema], completed_table_chain_jobs: Optional[Sequence[LoadJobInfo]] = None, ) -> List[NewLoadJob]:
        ...
        for table in table_chain:
            ...
            if table.get("write_disposition")=="merge":
                parent_table = table.get("parent")
                jobs.append(LanceDBRemoveOrphansJob(db_client=self.db_client, table_schema=self.prepare_load_table(table["name"]), fq_table_name=self.make_qualified_table_name(
                    table["name"]), fq_parent_table_name=(self.make_qualified_table_name(parent_table) if parent_table else None), client_config=self.config, ))
...
class LoadLanceDBJob(LoadJob, FollowupJob):
    ...
    def __init__(self, schema: Schema, table_schema: TTableSchema, local_path: str, type_mapper: LanceDBTypeMapper, db_client: DBConnection, client_config: LanceDBClientConfiguration, model_func: TextEmbeddingFunction, fq_table_name: str, fq_parent_table_name:
    Optional[str], ) -> None:
        ...
        self.write_disposition: TWriteDisposition = cast(TWriteDisposition, self.table_schema.get("write_disposition", "append"))
        ...
        if self.table_schema not in self.schema.dlt_tables():
            for record in records:
                ...
                uuid_id = (generate_uuid(record, self.unique_identifiers, self.fq_table_name) if self.unique_identifiers else str(uuid.uuid4()))
        ...
        upload_batch(records, db_client=db_client, table_name=self.fq_table_name, write_disposition=self.write_disposition, id_field_name=self.id_field_name, )
...
class LanceDBRemoveOrphansJob(NewLoadJobImpl):
    def __init__(self, db_client: DBConnection, table_schema: TTableSchema, client_config: LanceDBClientConfiguration, fq_table_name: str, fq_parent_table_name: Optional[str], ) -> None:
        self.db_client = db_client
        self.table_schema: TTableSchema = table_schema
        self.fq_table_name: str = fq_table_name
        self.fq_parent_table_name: Optional[str] = fq_parent_table_name
        self.write_disposition: TWriteDisposition = cast(TWriteDisposition, self.table_schema.get("write_disposition"))
        self.id_field_name: str = client_config.id_field_name

        job_id = ParsedLoadJobFileName(table_schema["name"], ParsedLoadJobFileName.new_file_id(), 0, "parquet", ).file_name()

        super().__init__(file_name=job_id, status="running", )
    ...
    def execute(self) -> None:
        ...
        if self.write_disposition!="merge":
            raise DestinationTerminalException(f"Unsupported write disposition {self.write_disposition} for LanceDB Destination"
        ...
        try:
            ...
            raise DestinationTransientException("Couldn't open lancedb database. Orphan removal WILL BE RETRIED") from e
        ...
        try:
            if self.fq_parent_table_name:
                ...
                grouping_key = (get_columns_names_with_prop(self.table_schema, DOCUMENT_ID_HINT) or self.id_field_name)
                grouping_key = grouping_key if isinstance(grouping_key, list) else [grouping_key]
                child_table_arrow: pa.Table = child_table.to_arrow().select([*grouping_key, "_dlt_load_id", "_dlt_id"])

                grouped = child_table_arrow.group_by(grouping_key).aggregate([("_dlt_load_id", "max")])
