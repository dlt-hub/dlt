...
class ClickHouseClusterCredentials(ClickHouseCredentials):
    ...
    __session_settings__: ClassVar[Dict[str, Any]] = ClickHouseCredentials.__session_settings__ | {
        ...
        # replicated tables; below settings increase consistency. We err on the side of correctness,
        # not performance.
        ...
        "insert_quorum": "auto",
        # Unfortunately, we have to allow non-deterministic mutations to support `merge` operations
        # (specifically those generated by `gen_delete_from_sql` and `gen_scd2_retire_sql`). As
        # opposed to above settings, this one weakens consistency guarantees.
