...
class SqlMergeJob(NewLoadJobImpl):
    ...
    def from_table_chain(
        ...
        try:
            ...
            sql = [
                " ".join(stmt.splitlines()) for stmt in cls.gen_merge_sql(table_chain, sql_client)
            ]
    ...
    def gen_delete_temp_table_sql(
        ...
        sql.append(
            f"CREATE TEMP TABLE {temp_table_name} AS SELECT"
            f" d.{unique_column} {key_table_clauses[0]};"
        )
    ...
    def gen_insert_temp_table_sql(
        ...
        sql.append(
            f"""CREATE TEMP TABLE {temp_table_name} AS
    ...
    def gen_merge_sql(
        ...
        if len(table_chain) == 1:
            ...
            create_delete_temp_table_sql, delete_temp_table_sql = cls.gen_delete_temp_table_sql(
            ...
            if primary_keys:
                create_insert_temp_table_sql, insert_temp_table_sql = cls.gen_insert_temp_table_sql(
        ...
        for table in table_chain:
            ...
            columns = ", ".join(
                map(sql_client.capabilities.escape_identifier, table["columns"].keys())
            )
            insert_sql = (
            ...
            if len(primary_keys) > 0:
                if len(table_chain) == 1:
                    ...
                    insert_sql += (
                        f" WHERE {uniq_column} IN (SELECT * FROM {insert_temp_table_sql});"
                    )
