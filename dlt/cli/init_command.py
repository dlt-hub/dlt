...
from dlt.cli import pipeline_files as files_ops
from dlt.cli import source_detection, utils
    ...
    if dependency_system:
        ...
        if dependency_system == utils.REQUIREMENTS_TXT:
            ...
            fmt.echo(
                ...
                % fmt.bold(f"pip3 install '{qs.join(source_files.requirements)}'")
            )
...
def init_command(
    ...
    if sources_storage.has_folder(source_name):
        ...
        remote_index = files_ops.get_remote_source_index(
            source_files.storage.storage_path, source_files.files
        )
        ...
        source_files = VerifiedSourceFiles(
            True, init_storage, pipeline_script, dest_pipeline_script, template_files, [], ""
        )
