...
def _read_csv_duckdb(
    ...
    **duckdb_kwargs: Any,
    ...
    for item in items:
        with item.open() as f:
            file_data = duckdb.from_csv_auto(cast(Any, f), **duckdb_kwargs)
...
if TYPE_CHECKING:
    ...
    class ReadersSource(DltSource):
        ...
        @copy_sig_any(_read_csv)
        def read_csv(self) -> DltResource: ...

        @copy_sig_any(_read_jsonl)
        def read_jsonl(self) -> DltResource: ...

        @copy_sig_any(_read_parquet)
        def read_parquet(self) -> DltResource: ...

        @copy_sig_any(_read_csv_duckdb)
