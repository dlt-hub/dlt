...
class DltResource(Iterable[TDataItem], DltResourceHints):
    ...
    def _remove_export_step(self) -> None:
        self._pipe.remove_by_type(RawDataExportWrapper)

    def _set_export_wrapper(self, wrapper: RawDataExportWrapper) -> None:
        self.add_step(wrapper)

    @property
    def raw_data_export(self) -> Optional[RawDataExportWrapper]:
        """Gets raw data export wrapper if it is in the pipe."""
        return self._pipe.get_by_type(RawDataExportWrapper)
    ...
    def _inject_config(self, incremental_from_hints_override: Optional[bool] = None) -> Self:
        ...
        if IncrementalResourceWrapper.should_wrap(sig):
            ...
            incr_f = gen
        # detect and wrap RawDataExporter argument
        if RawDataExportWrapper.should_wrap(sig):
            export_wrapper = RawDataExportWrapper()
            export_f = export_wrapper.wrap(sig, incr_f)
            self._set_export_wrapper(export_wrapper)
        else:
            export_f = incr_f
