...
class DltResourceSchema:
    ...
    def apply_hints(
        ...
        if not self._table_schema_template:
            ...
            t = self.new_table_template(
                table_name, parent_table_name, write_disposition, columns, primary_key, merge_key
            )
        else:
            # set single hints
    ...
    def new_table_template(
        ...
        if any(callable(v) for k, v in new_template.items() if k != "name") and not callable(
            table_name
        ):
