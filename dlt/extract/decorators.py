...
from dlt.common.configuration import configspec, get_fun_spec, known_sections, with_config
...
from dlt.common.typing import AnyFun, Concatenate, ParamSpec, TDataItem, TDataItems
    ...
    def decorator(f: Callable[TSourceFunParams, Any]) -> Callable[TSourceFunParams, DltSource]:
        ...
        if not schema:
            ...
            schema = _maybe_load_schema_for_callable(f, effective_name) or Schema(
                effective_name, normalize_name=True
            )
            ...
            with Container().injectable_context(SourceSchemaInjectableContext(schema)):
                ...
                with inject_section(
                    ConfigSectionContext(
                        pipeline_name=pipeline_name, sections=source_sections, source_state_key=name
                    )
                ):
