...
def _apply_lag_to_datetime(
    ...
    # NOTE: do not use timedelta with pendulum. do not use add and subtract. they destroy timezones
    if isinstance(value, pendulum.DateTime):
        # utc_lag = pendulum.from_timestamp(value.in_tz("UTC").timestamp() + lag)
        # r_v = utc_lag.in_tz(value.tzinfo) if value.tzinfo else utc_lag.naive()
        # print(value, "in", value.tz, "0---->", r_v, "|",  utc_lag)
