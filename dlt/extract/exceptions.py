...
class PipeNotBoundToData(PipeException):
    def __init__(self, pipe_name: str, has_parent: bool) -> None:
        ...
        if has_parent:
            msg = (
                f"A pipe created from transformer {pipe_name} is unbound or its parent"
                " is unbound or empty. Provide a resource in `data_from` argument or"
                " bind resources with | operator."
            )
        else:
...
class InvalidStepFunctionArguments(PipeException):
    def __init__(
        ...
        super().__init__(
            ...
            f" {func_name} requires first argument to take data item and optional"
            f" second argument named 'meta', but the signature is {sig}",
        )
...
class PipeGenInvalid(PipeException):
    def __init__(self, pipe_name: str, gen: Any) -> None:
        msg = (
            "A pipe generator element must be an Iterator (ie. list or generator"
            " function). Generator element is typically created from a `data` argument"
            " to pipeline.run or extract method."
        )
        msg += (
            " dlt will evaluate functions that were passed as data argument. If you"
            " passed a function the returned data type is not iterable. "
        )
...
class DynamicNameNotStandaloneResource(DltResourceException):
    def __init__(self, resource_name: str) -> None:
        super().__init__(
            resource_name,
            "You must set the resource as standalone to be able to dynamically set its"
            " name based on call arguments",
        )
...
class InvalidResourceDataType(DltResourceException):
    def __init__(
        ...
        super().__init__(
            resource_name,
            f"Cannot create resource {resource_name} from specified data. If you want"
            " to process just one data item, enclose it in a list. "
            + msg,
        )
...
class InvalidParallelResourceDataType(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            "Parallel resource data must be a generator or a generator function. The"
            f" provided data type for resource '{resource_name}' was {_typ.__name__}.",
        )
...
class InvalidResourceDataTypeBasic(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            f"Resources cannot be strings or dictionaries but {_typ.__name__} was"
            " provided. Please pass your data in a list or as a function yielding"
            " items. If you want to process just one data item, enclose it in a list.",
        )
...
class InvalidResourceDataTypeFunctionNotAGenerator(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            "Please make sure that function decorated with @dlt.resource uses 'yield'"
            " to return the data.",
        )
...
class InvalidResourceDataTypeMultiplePipes(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            "Resources with multiple parallel data pipes are not yet supported. This"
            " problem most often happens when you are creating a source with"
            " @dlt.source decorator that has several resources with the same name.",
        )
...
class InvalidTransformerDataTypeGeneratorFunctionRequired(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            "Transformer must be a function decorated with @dlt.transformer that takes"
            " data item as its first argument. Only first argument may be 'positional"
            " only'.",
        )
...
class InvalidTransformerGeneratorFunction(DltResourceException):
    def __init__(
        ...
        self.code = code
        msg = (
            f"Transformer function {func_name} must take data item as its first"
            " argument. "
        )
        if code == 1:
            ...
            msg += (
                "Only the first argument may be 'positional only', actual signature is"
                f" {sig}"
            )
        elif code == 3:
            msg += (
...
class ResourceInnerCallableConfigWrapDisallowed(DltResourceException):
    def __init__(self, resource_name: str, section: str) -> None:
        ...
        msg = (
            f"Resource {resource_name} in section {section} is defined over an inner"
            " function and requests config/secrets in its arguments. Requesting secret"
            " and config values via 'dlt.secrets.values' or 'dlt.config.value' is"
            " disallowed for resources that are inner functions. Use the dlt.source to"
            " get the required configuration and pass them explicitly to your source."
        )
        super().__init__(resource_name, msg)
...
class InvalidResourceDataTypeIsNone(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            "Resource data missing. Did you forget the return statement in"
            " @dlt.resource decorated function?",
        )
...
class ResourceFunctionExpected(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            "Expected function or callable as first parameter to resource"
            f" {resource_name} but {_typ.__name__} found. Please decorate a function"
            " with @dlt.resource",
        )
...
class InvalidParentResourceDataType(InvalidResourceDataType):
    def __init__(self, resource_name: str, item: Any, _typ: Type[Any]) -> None:
        super().__init__(
            ...
            item,
            _typ,
            f"A parent resource of {resource_name} is of type {_typ.__name__}. Did you"
            " forget to use '@dlt.resource` decorator or `resource` function?",
        )
...
class InvalidParentResourceIsAFunction(DltResourceException):
    def __init__(self, resource_name: str, func_name: str) -> None:
        ...
        super().__init__(
            resource_name,
            f"A data source {func_name} of a transformer {resource_name} is an"
            " undecorated function. Please decorate it with '@dlt.resource' or pass to"
            " 'resource' function.",
        )
...
class ParametrizedResourceUnbound(DltResourceException):
    def __init__(
        ...
        self.sig = sig
        msg = (
            f"The {kind} {resource_name} is parametrized and expects following"
            f" arguments: {sig}. Did you forget to bind the {func_name} function? For"
            f" example from `source.{resource_name}.bind(...)"
        )
        if error:
            msg += f" .Details: {error}"
        super().__init__(resource_name, msg)
...
class SourceDataIsNone(DltSourceException):
    def __init__(self, source_name: str) -> None:
        ...
        super().__init__(
            f"No data returned or yielded from source function {source_name}. Did you"
...
class SourceExhausted(DltSourceException):
    def __init__(self, source_name: str) -> None:
        self.source_name = source_name
        super().__init__(
            f"Source {source_name} is exhausted or has active iterator. You can iterate"
            " or pass the source to dlt pipeline only once."
        )
...
class ResourcesNotFoundError(DltSourceException):
    def __init__(
        ...
        msg = (
            ...
            f" {self.not_found_resources}. Available resources are:"
...
class SourceNotAFunction(DltSourceException):
    def __init__(self, source_name: str, item: Any, _typ: Type[Any]) -> None:
        ...
        self.typ = _typ
        super().__init__(
            f"First parameter to the source {source_name} must be a function or"
            f" callable but is {_typ.__name__}. Please decorate a function with"
            " @dlt.source"
        )
...
class SourceIsAClassTypeError(DltSourceException):
    def __init__(self, source_name: str, _typ: Type[Any]) -> None:
        ...
        self.typ = _typ
        super().__init__(
            f"First parameter to the source {source_name} is a class {_typ.__name__}."
            " Do not decorate classes with @dlt.source. Instead implement __call__ in"
            " your class and pass instance of such class to dlt.source() directly"
        )
...
class CurrentSourceSchemaNotAvailable(DltSourceException):
    def __init__(self) -> None:
        super().__init__(
            "Current source schema is available only when called from a function"
            " decorated with dlt.source or dlt.resource"
        )
...
class CurrentSourceNotAvailable(DltSourceException):
    def __init__(self) -> None:
        super().__init__(
            "Current source is available only when called from a function decorated"
            " with dlt.resource or dlt.transformer during the extract step"
        )
...
class ExplicitSourceNameInvalid(DltSourceException):
    def __init__(self, source_name: str, schema_name: str) -> None:
        ...
        super().__init__(
            f"Your explicit source name {source_name} is not a valid schema name."
            f" Please use a valid schema name ie. '{schema_name}'."
        )
...
class IncrementalUnboundError(DltResourceException):
    def __init__(self, cursor_path: str) -> None:
        super().__init__(
            "",
            f"The incremental definition with cursor path {cursor_path} is used without"
            " being bound to the resource. This most often happens when you create"
            " dynamic resource from a generator function that uses incremental. See"
