...
def verify_partial_table(schema: Schema, partial_table: TPartialTableSchema) -> None:
    ...
    if existing_table := schema.tables.get(table_name):
        if has_table_seen_data(existing_table):
            ...
            if root_key_col:
                logger.warning(
                    f"You are adding a root_key in column {root_key_col} to an already existing table {table_name}. "
                    "This will most likely fail due to NOT NULL constraint. Typically this happens when you switch "
                    "write disposition from append/replace to merge - in that case dlt will create required linking "
                    "information automatically. You can fix your table, drop it or disable root key propagation. "
                    "See details here: https://dlthub.com/docs/general-usage/merge-loading#switch-from-appendreplace-to-merge"
                )
