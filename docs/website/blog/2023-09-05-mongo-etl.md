...
Here's a code explanation of how it work under the hood:
...
print(load_info)
```
4. Now we can use the data, these are the tables:



 *json_data*

| index | id   | name      | job__company | job__title      | _dlt_load_id        | _dlt_id         |
| ----- |------| --------- | ------------ | --------------- | ------------------- | --------------- |
| 0     | 1    | Alice     | ScaleVector  | Data Scientist  | 1693922245.602667   | 0ZbCzK7Ra2tWMQ  |


and  *json_data__children*
...
Note that the original json got unpacked into tables that are now
joinable via generated keys `child._dlt_parent_id = parent._dlt_id`.
