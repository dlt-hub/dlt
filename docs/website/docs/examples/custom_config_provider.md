...
# set the active profile explicitly (normally this comes from config.toml or an env var)
dlt.config["dlt_config_profile_name"] = "prod"

# dlt standard providers work at this point (we have the profile name in config)
...
# the registered provider now resolves config and secrets for the github_api source
assert dlt.config["sources.github_api.github.url"] == "https://github.com/api"
assert dlt.secrets["sources.github_api.github.api_key"] == "secret_key"
```
