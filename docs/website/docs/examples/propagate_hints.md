...
# the primary key `parent_id` is propagated into the nested `children` table
row_counts = pipeline.last_trace.last_normalize_info.row_counts
assert row_counts["parent_with_children"] == 2
assert row_counts["parent_with_children__children"] == 3
```
