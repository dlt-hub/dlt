...
- **Compact `sources` config lookup.** Source config now also resolves a compact `sources.<name>.<key>` path in addition to the full `sources.<section>.<name>.<key>` ([#3626](https://github.com/dlt-hub/dlt/pull/3626)). The full path and the section path still take precedence. This is breaking if you stored unrelated values at `sources.<name>`, as they are now resolved for the matching source.

## AI Workbench for coding agents

The AI Workbench ships as the `dlthub ai` command group, installed with `pip install dlt[hub]` ([#3674](https://github.com/dlt-hub/dlt/pull/3674)). Run `dlthub ai init` to configure a coding agent (Claude Code, Cursor, or Codex) with dlt rules and skills, install curated toolkits with `dlthub ai toolkit`, and run a pluggable MCP server with `dlthub ai mcp`. In the base package, `dlt ai` redirects here.
