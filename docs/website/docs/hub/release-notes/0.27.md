...
[Workspaces](../pipeline-operations/what-is-a-workspace.md) can now be archived from a new **Danger Zone** in workspace settings. This is the web UI for the archiving that 0.26 shipped through the API.
...
Job names also dropped the deployment-file suffix, so `chinook_invoice (__deployment__)` now reads `chinook_invoice`, or whatever display name the author set.
