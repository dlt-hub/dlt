...
| Agent | A system prompt with declared inputs, output, and access that the dltHub platform runs unattended as a job | `job-inspector` diagnoses a failed job run (see [Agents](../agents/index.md)) |
