...
- Additionally, the dlthub dbt generator lets you define relationships between the schema tables, which can be used to automatically create fact tables.
...
```sh
dlthub dbt generate <pipeline-name>
```
...
```sh
dlthub dbt generate <pipeline-name> --fact <fact_table_name>
```
...
```sh
dlthub dbt generate example_shop --fact orders
```
...
You can run your dbt project with the previously mentioned script that was generated by `dlthub dbt generate <pipeline-name>`:
