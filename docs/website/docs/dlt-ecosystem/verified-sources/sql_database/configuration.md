...
`sql_database_source` configures each table with a dictionary instead of Python code. It is
useful when tables need different hints (write disposition, primary key, incremental, renamed
tables) and when the configuration is written by someone else, generated, or kept in a file.

A table is declared as a table name or as a dictionary whose fields are the arguments of
`sql_table` plus the [table hints](../../../general-usage/resource.md#adjust-schema) of
`dlt.resource`. `table_defaults` are applied to every table and are overridden by the settings
of a single table.
...
source = sql_database_source({
    # credentials are resolved from `secrets.toml` or the environment when omitted
    "credentials": "mysql+pymysql://rfamro@mysql-rfam-public.ebi.ac.uk:4497/Rfam",
    "table_defaults": {
        "write_disposition": "replace",
    },
    "tables": [
        # a table name loads the table with the default settings
        "clan",
        {
            # `name` is the name of the resource, `table` the name of the table in the database
            "name": "families",
            "table": "family",
            # `table_name` is the name of the table in the destination
            "table_name": "rfam_families",
            "included_columns": ["rfam_acc", "rfam_id", "updated"],
...
The configuration is a plain dictionary of JSON compatible values, so it may be loaded from a
TOML, YAML or JSON file, or generated by an agent. Adapters and other callables are accepted as
well but they make the configuration non-serializable.

Like `sql_database`, all tables of the database schema are loaded when `tables` is omitted. Such
tables are configured with `table_defaults`, so the configuration below loads the whole database
with `replace` write disposition. Set `"include_views": True` to discover views as well, and
declare `"tables": []` to load nothing.
...
Use `sql_database_resources` instead of `sql_database_source` to get the list of resources ie.
to add them to your own source. It takes the same configuration but requires `credentials` to be
present in it.

:::note
`tables` also accepts ready resources, ie. from `sql_table`, so a table that needs Python code
may still be declared next to the other tables.
:::
