...
- _BigQuery Data Editor_
- _BigQuery Job User_
- _BigQuery Read Session User_
...
The following table shows the key capabilities of the BigQuery destination:
...
Note that `bigquery_adapter` updates the resource _in place_, but returns the resource for convenience, i.e., both the following are valid:
