...
**Note:** If the S3 bucket is in a different region than the Redshift cluster, you must set the region_name value in the [destination.filesystem.credentials] in the config.toml file to ensure proper access. Additionally, Redshift does not support cross-region COPY operations for columnar storage formats, such as Parquet.
