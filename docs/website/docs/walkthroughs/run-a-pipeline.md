...
an exception on failed jobs and abort the package**. Aborted packages cannot be retried. If you want
to decide yourself whether to retry, fail, or abort packages with terminally failing jobs, see
[how to manage pending packages manually](../running-in-production/running.md#retry-fail-or-abort-pending-packages-manually).
