...
| 5. | Merge based on date | Merges records based on a date field and adds new ones. |
...
2. The Python code illustrates the process of loading data from an SQL source into BigQuery using the `dlt` pipeline in "append" mode using `created_at` as an incremental parameter:
...
2. The Python code illustrates the process of loading data from an SQL source into BigQuery using the `dlt` pipeline in "merge" mode using `last_modified_at` as an incremental parameter:
