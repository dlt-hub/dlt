...
| Freeze column types | **`data_type` Contract:** Set `schema_contract={"data_type": "freeze"}` to raise an error when a variant column is created or when `data_type`, `nullable`, `precision`, `scale`, or `timezone` changes on an existing column. | [Schema Contracts](schema-contracts.md#setting-up-the-contract) | dlt |
...
| Find orphan keys | **Post-Load Assertions:** Run SQL tests on the destination to identify child records missing a valid parent (for example, Orders without Customers). | [SQL Transformations](../dlt-ecosystem/transformations/sql.md) | dlt |
...
| Collect custom metrics | **Custom Metrics:** Track business-specific statistics during extraction (for example, page counts, API call counts) using `dlt.current.resource_metrics()`. | [Resource](resource.md#collect-custom-metrics) | dlt |
