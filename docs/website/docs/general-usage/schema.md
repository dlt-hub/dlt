...
typically JSON documents, contains identifiers (ie. key names in a dictionary) with any Unicode
...
> 💡 If you provide any schema elements that contain identifiers via decorators or arguments (ie.
...
conventions that ie. pass all the identifiers unchanged if the destination accepts that (ie.
duckdb).
...
The data normalizer is configurable and users can plug their own normalizers ie. to handle the
...
The `settings` section of schema file let's you define various global rules that impact how tables
...
side is the data type. ❗See the column hint rules for naming convention!
...
What if you can configure your schema or add some tables only inside your schema function, when ie.
