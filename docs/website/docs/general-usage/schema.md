...
To stop variant columns from being created, set the `data_type` entity of the [schema contract](schema-contracts.md#setting-up-the-contract) to a mode other than `evolve`. The same contract also governs changes to the type of a column that is already complete: `data_type`, `nullable`, `precision`, `scale`, and `timezone`.
