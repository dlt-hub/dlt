...
password="pass"
```
The snippet above demonstrates how to apply certain naming for an example `zendesk` source.
...
:::note
Note that a fully qualified name of your custom naming convention will be stored in the `Schema` and `dlt` will attempt to import it when the schema is loaded from storage.
