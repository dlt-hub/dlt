...
A [resource](general-usage/glossary.md#resource) is a function that yields data. To create a
...
> 💡 You can mark some of resource arguments as configuration and [credentials](./credentials.md)
...
`generate_row` resource to generate the number of rows we request
...
You can mark some of resource arguments as configuration and [credentials](./credentials.md) values
...
that returns a list of objects (ie. users) in one endpoint and user details in another. You can deal
...
table, you may want to just sample a fragment of it in order ie. to quickly see the dataset with
