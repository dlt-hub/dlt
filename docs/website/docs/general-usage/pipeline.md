---
last-grammar-check: 2024-02-26T13:08:57.142797+01:00
title: Pipeline
...
Once the pipeline runs, all resources get evaluated, and the data is loaded at the destination.

Example:

This pipeline will load a list of objects into a `duckdb` table with the name "three":
...
- `destination` a name of the [destination](../dlt-ecosystem/destinations) to which `dlt`
  will load the data. May also be provided to the `run` method of the `pipeline`.
- `dataset_name` a name of the dataset to which the data will be loaded. A dataset is a logical
  group of tables i.e. `schema` in relational databases or a folder grouping many files. May also be
...
- `table_name` - specified in case when the table name cannot be inferred i.e. from the resources or name
...
location for such folders is in the user home directory: `~/.dlt/pipelines/<pipeline_name>`.
