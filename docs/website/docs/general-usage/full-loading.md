...
The [lance](../dlt-ecosystem/destinations/lance.md), [LanceDB](../dlt-ecosystem/destinations/lancedb.md), [Qdrant](../dlt-ecosystem/destinations/qdrant.md), and [Weaviate](../dlt-ecosystem/destinations/weaviate.md) destinations support `truncate-and-insert` only, so on those any other configured strategy fails this way. A destination may also reject a strategy for reasons it can only check against the live database. ClickHouse `staging-optimized`, for example, requires the Atomic or Shared database engine.
