...
See [Schema contract](../general-usage/schema-contracts.md) for the current scope of the `tables`, `columns`, and `data_type` contracts - the `data_type` contract covers variant columns as well as changes to the type of a column that already exists in the schema.
