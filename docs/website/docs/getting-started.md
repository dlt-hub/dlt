...
```toml
Take example above that reads files from folder and make it a resource that returns list of filenames and last modification times

Add `map` transform to the resources that will load each file, convert PDF to text.

Apply weaviate adapter to vectorize text

Load to weaviate

We should be able to write that less than 20 lines
```
...
Parsing happens in `@dlt.transformer` which receives data from `list_files` resource. The `weaviate_adapter` is used to tell `weaviate` destination which fields to vectorize.
