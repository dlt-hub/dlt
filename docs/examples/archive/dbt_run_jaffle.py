...
print(
    "create or restore virtual environment in which dbt is installed, use the newest"
    " version of dbt"
)
...
for m in models:
    print(
        f"Model {m.model_name} materialized in {m.time} with status {m.status} and"
        f" message {m.message}"
    )
...
for m in models:
    print(
        f"Test {m.model_name} executed in {m.time} with status {m.status} and message"
        f" {m.message}"
    )
