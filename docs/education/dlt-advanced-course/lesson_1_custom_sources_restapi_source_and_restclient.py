...
def _(dlt):
        ...
        "resources": [
            ...
            },
            {
                ...
                "endpoint": {
                    ...
                    "params": {
                        "issue_number": {"type": "resolve", "resource": "issues", "field": "number"}
                    },
                },
            },
    ...
    _load_info = pipeline_1.run(git_source)
    print(_load_info)
...
@app.cell
def _(BearerTokenAuth, HeaderLinkPaginator, RESTClient, dlt, os, userdata):
...
@app.cell
def _(RESTClient, userdata):
    from dlt.sources.helpers.rest_client.auth import APIKeyAuth

    _api_key = userdata.get("NEWS_API_KEY")
    client_1 = RESTClient(
        base_url="https://newsapi.org/v2/",
        auth=APIKeyAuth(name="apiKey", api_key=_api_key, location="query"),
    )
    response = client_1.get("everything", params={"q": "python", "page": 1})
    print(response.json())
    return APIKeyAuth, client_1


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""This authenticates every request by adding `?apiKey=your_key` to the URL.""")
    return
...
@app.cell
def _(client_1):
    page_iterator = client_1.paginate("everything", params={"q": "python", "page": 1})
...
@app.cell
def _(APIKeyAuth, RESTClient, userdata):
...
def _(DltResource, Iterable, dlt, get_articles, get_top_articles):
    ...
    def newsapi_source(api_key: str = dlt.secrets.value) -> Iterable[DltResource]:
        return [get_articles(api_key=_api_key), get_top_articles(api_key=_api_key)]
...
def _(dlt, newsapi_source):
    ...
    info = pipeline_2.run(newsapi_source())
    print(info)
    return (pipeline_2,)
...
@app.cell
def _(pipeline_2):
    pipeline_2.dataset().python_articles.df().head()
    return


@app.cell
def _(pipeline_2):
    pipeline_2.dataset().top_articles.df().head()
    return
...
def _(RESTAPIConfig, dlt, rest_api_source):
    _news_config: RESTAPIConfig = {
        ...
        "resources": [
            {"name": "news_articles", "endpoint": {"path": "everything", "params": {"q": "python"}}}
        ],
    }
    ...
    _load_info = pipeline_3.run(_news_source)
...
@app.cell
def _(dlt, rest_api_source, userdata):
    _api_key = userdata.get("NEWS_API_KEY")
    _news_config = {
        "client": {
            "base_url": "https://newsapi.org/v2/",
            "auth": {"type": "api_key", "name": "apiKey", "api_key": _api_key, "location": "query"},
        },
        "resources": [
            {"name": "news_articles", "endpoint": {"path": "everything", "params": {"q": "python"}}}
        ],
    ...
    _load_info = pipeline_4.run(_news_source)
...
def _(dlt, rest_api_source, userdata):
    ...
    _load_info = pipeline_5.run(_news_source)
...
@app.cell
def _(pipeline_5):
    pipeline_5.dataset().news_articles.df().head()
    return
...
def _(dlt, rest_api_source, userdata):
    ...
    _load_info = pipeline_6.run(_news_source)
...
def _(dlt, rest_api_source, userdata):
    ...
    _load_info = pipeline_7.run(_news_source)
...
def _(dlt, rest_api_source, userdata):
    ...
    _load_info = pipeline_8.run(_news_source)
...
def _(dlt, rest_api_source, userdata):
    ...
    _load_info = pipeline_9.run(_news_source)
    print(pipeline_9.last_trace)
    pipeline_9.dataset().top_headlines.df().head()
    return
...
@app.cell
def _(Any, requests):
    def debug_response(response: requests.Response, *args: Any, **kwargs: Any) -> requests.Response:
...
@app.cell
def _(debug_response, dlt, rest_api_source, userdata):
    _api_key = userdata.get("NEWS_API_KEY")
    _news_config = {
        "client": {
            "base_url": "https://newsapi.org/v2/",
        ...
        "resources": [
            {
                ...
                "endpoint": {
                    "path": "everything",
                    "response_actions": [{"status_code": 200, "action": debug_response}],
                    "params": {
                        "q": "python",
                        "from": {
                            "type": "incremental",
    ...
    _load_info = pipeline_10.run(_news_source)
    print(pipeline_10.last_trace)
    pipeline_10.dataset().news_articles.df().head()
    return
...
@app.cell
def _(debug_response, dlt, lower_title, rest_api_source, userdata):
    _api_key = userdata.get("NEWS_API_KEY")
    _news_config = {
        "client": {
            "base_url": "https://newsapi.org/v2/",
    ...
    _load_info = pipeline_11.run(_news_source)
    print(pipeline_11.last_trace)
    pipeline_11.dataset().news_articles.df().head()
    return (pipeline_11,)
...
def _(mo):
    mo.md(
        r"""✅ ▶ Well done! Go to [the next lesson.](https://colab.research.google.com/drive/1lQ8VkrGJwZMsVtbkuYympcvbv0_CCgYo#forceEdit=true&sandboxMode=true)"""
    )
    return
