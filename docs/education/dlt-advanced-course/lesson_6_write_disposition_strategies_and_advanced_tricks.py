...
def _():
    ...
    pipeline = dlt.pipeline(
        pipeline_name="pokemon_load_1", destination="duckdb", dataset_name="pokemon_data_1"
    )
    _load_info = pipeline.run(data, table_name="pokemon", write_disposition="replace")
    print(pipeline.last_trace)  # <--- set the replace strategy using TOML, ENVs or Python
    return dlt, pipeline
...
def _(dlt):
    ...
    pipeline_1 = dlt.pipeline(
        pipeline_name="pokemon_load_2", destination="duckdb", dataset_name="pokemon_data_2"
    )
    _load_info = pipeline_1.run(data_1, table_name="pokemon", write_disposition="replace")
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""We see that the staging table contains all the data we loaded.""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Load this data into duckdb with merge write disposition.""")
    return
...
def _(data_2, dlt):
    ...
    pipeline_2 = dlt.pipeline(
        pipeline_name="poke_pipeline_merge", destination="duckdb", dataset_name="pokemon_data"
    )
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""### **I. `delete-insert` strategy**""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Run the pipeline again:""")
    return
...
def _(mo):
    mo.md(
        r"""We see agian the staging schema called `pokemon_data_staging`. Let's check the content:"""
    )
    return
...
def _(mo):
    mo.md(
        r"""***Not supported in DuckDB.** List of supported destinations can be found in [docs](https://dlthub.com/docs/general-usage/incremental-loading#upsert-strategy)."""
    )
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Now, run the pipeline with merge disposition and SCD2 strategy:""")
    return
...
def _(TDataItems, data_4, dlt):
    ...
    pipeline_3 = dlt.pipeline(
        pipeline_name="pokemon_pipeline", destination="duckdb", dataset_name="pokemon_scd2"
    )
...
def _(mo):
    mo.md(
        r"""Modify the dataset by changing Pikachu weight again. This simulates a change in source data that should be tracked by SCD2:"""
    )
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Run the pipeline again with the modified dataset:""")
    return
...
def _(TDataItems, data_6, dlt):
    ...
    pipeline_4 = dlt.pipeline(
        pipeline_name="pokemon_pipeline", destination="duckdb", dataset_name="pokemon_hd"
    )
...
def _():
    data_7 = [
        {"id": "25", "name": "pikachu", "size": {"weight": 6, "height": 0.4}, "deleted_flag": True}
    ]  # <--- set to True
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Pikachu record was deleted from loaded data.""")
    return
...
def _():
    data_8 = [
        {"id": "25", "name": "pikachu", "size": {"weight": 6, "height": 0.4}, "deleted_flag": None},
        {"id": "25", "name": "pikachu", "size": {"weight": 7, "height": 0.4}, "deleted_flag": True},
        {"id": "25", "name": "pikachu", "size": {"weight": 8, "height": 0.4}, "deleted_flag": None},
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r""" This will insert one record (the one with size__weight = 8).""")
    return
...
def _(TDataItems, data_8, dlt):
    @dlt.resource(
        ...
        write_disposition="merge",
        primary_key="id",
        columns={"deleted_flag": {"hard_delete": True}, "size__weight": {"dedup_sort": "desc"}},
    )
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""The row with the largest value of "size__weight" 8.0 remains.""")
    return
...
def _(TDataItems, data_9, dlt):
    ...
    pipeline_6 = dlt.pipeline(
        pipeline_name="pokemon_pipeline", destination="duckdb", dataset_name="pokemon_inc"
    )
...
def _(TDataItem, TDataItems, data_10, dlt):
    ...
    def set_default_updated_at(record: TDataItem) -> TDataItems:
        ...
        return record  # <--- use 'created_at' instead of missing 'updated_at'
...
def _(dlt, with_default_values):
    pipeline_7 = dlt.pipeline(
        pipeline_name="pokemon_pipeline_wd", destination="duckdb", dataset_name="pokemon_inc_wd"
    )
...
def _(dlt, without_none):
    pipeline_8 = dlt.pipeline(
        pipeline_name="pokemon_pipeline_wn", destination="duckdb", dataset_name="pokemon_inc_wn"
    )
...
def _(mo):
    mo.md(
        r"""Note that dlt's incremental filtering considers the ranges half-closed. `initial_value` is inclusive, `end_value` is exclusive, so chaining ranges like above works without overlaps. This behaviour can be changed with the `range_start` (default `"closed"`) and `range_end` (default `"open"`) arguments."""
    )
    return
...
def _(mo):
    mo.md(
        r"""Specifically for the `sql_database` source you can utilize another possible approach - load data in fixed chunks using `chunk_size` parameter."""
    )
    return
...
def _():
    # magic command not supported in marimo; please file an issue to add support
    # %%capture
    # # (use marimo's built-in package management features instead) !pip install dlt pymysql
    return
...
def _(dlt):
    ...
    while continue_load_flag:
        ...
        continue_load_flag = (
            my_table_name in pipeline_10.last_trace.last_normalize_info.row_counts.keys()
        )
...
def _(mo):
    mo.md(
        r"""![Lesson_6_Write_disposition_strategies_%26_Advanced_tricks_img1](https://storage.googleapis.com/dlt-blog-images/dlt-advanced-course/Lesson_6_Write_disposition_strategies_%26_Advanced_tricks_img1.png)"""
    )
    return
...
def _(mo):
    mo.md(
        r"""✅ ▶ Proceed to the [next lesson](https://colab.research.google.com/drive/1mC09rjkheo92-ycjjq0AlIzgwJC8-ZMX#forceEdit=true&sandboxMode=true)!"""
    )
    return
