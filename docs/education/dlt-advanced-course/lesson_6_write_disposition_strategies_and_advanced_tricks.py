...
def _(mo):
    mo.md(
        r"""In this example, we're using the `replace_strategy="truncate-and-insert"` parameter in the pipeline method to indicate that we want to use the `truncate-and-insert` strategy for replacing data."""
    )
    return
...
def _(mo):
    mo.md(
        r"""Let's remember our Pokemon data sample from the dlt Fundamentals course:"""
    )
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Imagine that we want to load only updated data:""")
    return
...
@app.cell
def _(data_3, pipeline_2, pokemon):
    _load_info = pipeline_2.run(pokemon(data_3))
    print(_load_info)
    pipeline_2.dataset().pokemon.df()
    return
...
@app.cell
def _(data_5, pipeline_3, pokemon_1):
    _load_info = pipeline_3.run(pokemon_1(data_5))
    print(_load_info)
    return
...
def _(mo):
    mo.md(
        r"""SCD2 created a new row for Pikachu with updated `size_weight` to 6.0 while keeping the historical record."""
    )
    return
...
@app.cell
def _(data_7, pipeline_4, pokemon_2):
    _load_info = pipeline_4.run(pokemon_2(data_7))
    print(_load_info)
    return
...
def _(mo):
    mo.md(
        r"""✅ ▶ Proceed to the [next lesson](https://github.com/dlt-hub/dlt/blob/master/docs/education/dlt-advanced-course/lesson_7_data_contracts.ipynb)!"""
    )
    return
