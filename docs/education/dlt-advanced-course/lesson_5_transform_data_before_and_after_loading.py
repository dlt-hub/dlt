...
def _():
    ...
    # %%capture
    # # (use marimo's built-in package management features instead) !pip install -U "dlt[sql_database, duckdb]"
...
def _(pipeline):
    with pipeline.sql_client() as _client:
        with _client.execute_query("SELECT * FROM genome") as _table:
            genome = _table.df()
    genome
    return
...
def _(pipeline):
    with pipeline.sql_client() as _client:
        with _client.execute_query("SELECT COUNT(*) AS total_rows FROM genome") as _table:
            print(_table.df())
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## **1. Filtering the data during the ingestion with `query_adapter_callback`**""")
    return
...
def _(mo):
    mo.md(
        r"""Imagine a use-case where we're only interested in getting the genome data for bacterias. In this case, ingesting the whole `genome` table would be an unnecessary use of time and compute resources."""
    )
    return
...
def _():
    ...
    def query_adapter_callback(query: SelectAny, table: Table) -> SelectAny:
        if _table.name == "genome":
            return query.where(_table.c.kingdom == "bacteria")
        return query  # Only select rows where the column kingdom has value "bacteria"  # Use the original query for other tables
...
def _(dlt, query_adapter_callback, sql_database):
    ...
    _load_info = pipeline_1.run(_source, write_disposition="replace")
...
def _(SelectAny, SelectClause, Table, dlt, sql_database):
    ...
    def query_adapter_callback_1(query: SelectAny, table: Table) -> SelectClause:
        if _table.name == "genome":
            return sa.text(f"SELECT * FROM {_table.fullname} WHERE kingdom='bacteria'")
        return query
...
def _(pipeline_2):
    with pipeline_2.sql_client() as _client:
        with _client.execute_query("SELECT DISTINCT author FROM clan LIMIT 5") as _table:
            print("Table clan:")
            print(_table.df())
    return
...
def _():
    ...
    def pseudonymize_name(row: TDataItem) -> TDataItem:
        ...
        salted_string = _row["author"] + salt  # add a constant salt to generate
        sh = hashlib.sha256()
        sh.update(salted_string.encode())
        hashed_string = sh.digest().hex()
        _row["author"] = hashed_string
        return _row
...
def _(dlt, sql_database):
    ...
    _load_info = pipeline_5.run(_source, write_disposition="replace")
...
def _(dlt, sql_database):
    ...
    _load_info = pipeline_6.run(_source, write_disposition="replace")
...
def _(TDataItem, TDataItems, dlt, sql_database):
    ...
    _info = pipeline_7.run([genome_resource, genome_resource | batch_stats])
...
def _(pipeline_7):
    with pipeline_7.sql_client() as _client:
        with _client.execute_query("SELECT * FROM batch_stats") as _table:
            res = _table.df()
    res
    return
...
def _(pipeline_7):
    with pipeline_7.sql_client() as _client:
        ...
        with _client.execute_query("SELECT * FROM genome_length") as _table:
            genome_length = _table.df()
    genome_length
    return
...
@app.cell
def _():
    # (use marimo's built-in package management features instead) !pip install ibis-framework[duckdb]
    return
...
def _(pipeline_7):
    ...
    _table = ibis_connection.table("batch_stats", database=dataset_name)
    print(_table.limit(2).execute())
    return
...
def _(mo):
    mo.md(
        r"""✅ ▶ Proceed to the [next lesson](https://colab.research.google.com/drive/1XT1xUIQIWj0nPWOmTixThgdXzi4vudce#forceEdit=true&sandboxMode=true)!"""
    )
    return
