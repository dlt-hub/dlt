...
def _():
    ...
    source = sql_database('mysql+pymysql://rfamro@mysql-rfam-public.ebi.ac.uk:4497/Rfam', table_names=['family'], query_adapter_callback=limit_rows)
    return os, source
...
def _(TDataItems, TTableSchema, dlt, os):
    ...
    def push_to_notion(items: TDataItems, table: TTableSchema, notion_auth: str=dlt.secrets.value, notion_page_id: str=dlt.secrets.value) -> None:
        client = Client(auth=notion_auth)
        print(len(items))
        for item in items:
            client.pages.create(parent={'database_id': notion_page_id}, properties={'Accession': {'title': [{'text': {'content': item['rfam_acc']}}]}, 'ID': {'rich_text': [{'text': {'content': item['rfam_id']}}]}, 'Description': {'rich_text': [{'text': {'content': item['description']}}]}})
...
def _(dlt, push_to_notion, source):
    _pipeline = dlt.pipeline('notion_pipeline', destination=push_to_notion, progress='log')
