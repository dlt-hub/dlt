...
def _():
    ...
    table_pipeline = dlt.pipeline(pipeline_name='data_contracts_table_level', destination='duckdb', dataset_name='mydata')
...
@app.cell
def _(dlt):
    column_pipeline = dlt.pipeline(pipeline_name='data_contracts_column_level', destination='duckdb', dataset_name='mydata')
...
def _(TDataItems, column_pipeline, dlt, duckdb):
    ...
    def allow_new_columns(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = column_pipeline.run(allow_new_columns([{'id': 2, 'name': 'Bob', 'age': 35}]), table_name='users')
    print(_load_info)
...
def _(TDataItems, column_pipeline, dlt, duckdb):
    ...
    def _discard_row(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = column_pipeline.run(_discard_row([{'id': 3, 'name': 'Sam', 'age': 30}, {'id': 4, 'name': 'Kate', 'age': 79, 'phone': '123-456-7890'}]), table_name='users')
    print(_load_info)
    # Attempt to load two additional rows. Only the row that follows the existing schema will be loaded
    print('\n')
    _conn = duckdb.connect(f'{column_pipeline.pipeline_name}.duckdb')
    # View the data that has been loaded
    _conn.sql('SELECT * FROM mydata.users').df()  # This row will be loaded  # This row will not be loaded
    return
...
def _(TDataItems, column_pipeline, dlt, duckdb):
    ...
    def _discard_value(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = column_pipeline.run(_discard_value([{'id': 5, 'name': 'Sarah', 'age': '23'}, {'id': 6, 'name': 'Violetta', 'age': '22', 'phone': '666-513-4510'}]), table_name='users')
    print(_load_info)
...
def _(TDataItems, column_pipeline, dlt):
    ...
    def no_new_columns(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = column_pipeline.run(no_new_columns([{'id': 7, 'name': 'Lisa', 'age': 40, 'phone': '098-765-4321'}]), table_name='users')
...
@app.cell
def _(dlt, duckdb):
    data_type_pipeline = dlt.pipeline(pipeline_name='data_contracts_data_type', destination='duckdb', dataset_name='mydata')
...
def _(TDataItems, data_type_pipeline, dlt, duckdb):
    ...
    def allow_any_data_type(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = data_type_pipeline.run(allow_any_data_type([{'id': 2, 'name': 'Bob', 'age': '35'}]), table_name='users')
    print(_load_info)
...
def _(TDataItems, data_type_pipeline, dlt, duckdb):
    ...
    def _discard_row(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = data_type_pipeline.run(_discard_row([{'id': 3, 'name': 'Sam', 'age': '35'}, {'id': 4, 'name': 'Kate', 'age': 'seventy'}]), table_name='users')
    print(_load_info)
...
def _(TDataItems, data_type_pipeline, dlt, duckdb):
    ...
    def _discard_value(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = data_type_pipeline.run(_discard_value([{'id': 5, 'name': 'Sarah', 'age': 23}, {'id': 6, 'name': 'Violetta', 'age': 'twenty-eight'}]), table_name='users')
    print(_load_info)
    # Load two additional rows. Since we're using the "discard_value" resource, both rows will be added
...
def _(TDataItems, data_type_pipeline, dlt):
    ...
    def no_data_type_changes(input_data: TDataItems) -> TDataItems:
        yield input_data
    _load_info = data_type_pipeline.run(no_data_type_changes([{'id': 7, 'name': 'Lisa', 'age': 'forty'}]), table_name='users')
