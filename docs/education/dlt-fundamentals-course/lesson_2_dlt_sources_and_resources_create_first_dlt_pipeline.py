...
def _(mo):
    mo.md(
        r"""2. Create and run a pipeline to fetch data from an SQL resource and query the loaded data as follows:"""
    )
    return
...
def _(all_data, dlt):
    ...
    _load_info = pipeline_1.run(all_data())
    print(_load_info)
    return
...
def _(TDataItems, dlt, requests):
    ...
    @dlt.resource(table_name="pokemon")
    def my_dict_list_2() -> TDataItems:
        yield data_1

    @dlt.transformer(data_from=my_dict_list_2, table_name="detailed_info")
    ...
    )  # <--- `items` is a variable and contains data from `my_dict_list` resource  # <-- print what data we get from `my_dict_list` source  # <--- print what data we get from API
    return data_1, pipeline_2, poke_details
...
@app.cell
def _(pipeline_2, poke_details):
    _load_info = pipeline_2.run(poke_details())
    print(_load_info)
    return
...
@app.cell
def _(TDataItem, TDataItems, data_1, dlt, pipeline_2, requests):
    @dlt.resource(table_name="pokemon")
    def my_dict_list_3() -> TDataItems:
        yield from data_1

    @dlt.transformer(data_from=my_dict_list_3, table_name="detailed_info")
    ...
    _load_info = pipeline_2.run(details())
    print(_load_info)
    return details, my_dict_list_3
...
@app.cell
def _(details, my_dict_list_3, pipeline_2):
    _load_info = pipeline_2.run(my_dict_list_3 | details)
    return
...
def _(pipeline_2):
    ...
    pipeline_2.dataset().detailed_info.df()
    return
...
def _(DltResource, Iterable, TDataItems, dlt, requests):
    ...
    _load_info = pipeline_3.run(github_data())
    print(_load_info)
    return
...
def _(mo):
    mo.md(
        r"""✅ ▶ Proceed to the [next lesson](https://colab.research.google.com/drive/1-jVNzMJTRYHhbRlXgGFlhMwdML1L9zMx#forceEdit=true&sandboxMode=true)!"""
    )
    return
