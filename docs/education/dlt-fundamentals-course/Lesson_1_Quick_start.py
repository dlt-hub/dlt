...
def _(mo):
    mo.md(
        r"""[Install](https://dlthub.com/docs/reference/installation) `dlt` with DuckDB as destination:"""
    )
    return
...
def _(dlt):
    pipeline_1 = dlt.pipeline(
        pipeline_name="resource_source", destination="duckdb", dataset_name="mydata", dev_mode=True
    )
    return (pipeline_1,)
...
@app.cell
def _(data, pipeline_1):
    _load_info = pipeline_1.run(data, table_name="pokemon")
    print(_load_info)
    return
...
def _(mo):
    mo.md(
        r"""Start a connection to your database using native `duckdb` connection and look what tables were generated:"""
    )
    return
...
def _(pipeline_1):
    ...
    conn = duckdb.connect(f"{pipeline_1.pipeline_name}.duckdb")
    conn.sql(f"SET search_path = '{pipeline_1.dataset_name}'")
...
def _(mo):
    mo.md(
        r"""✅ ▶ Proceed to the [next lesson](https://colab.research.google.com/drive/1tc94GvIoYXmYrjUibDhY_9iPR5zA0Eyw#forceEdit=true&sandboxMode=true)!"""
    )
    return
