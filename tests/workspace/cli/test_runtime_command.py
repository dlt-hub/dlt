...
def make_valid_jwt(email: str = "user@example.com", user_id: Optional[str] = None) -> str:
    payload = {"email": email, "sub": user_id or str(uuid.uuid4())}
...
def oauth_success_ctx(token: str):
    with (
        patch("dlt._workspace.cli._runtime_command.get_auth_client", return_value=object()),
        patch(
...
def test_runtime_login_api_exception_me_failure(script_runner: ScriptRunner) -> None:
    token = make_valid_jwt()
...
def test_runtime_login_with_valid_existing_token_skips_oauth(script_runner: ScriptRunner) -> None:
    ...
    with me_response_ctx(workspace_id):
        with (
            patch("dlt._workspace.cli._runtime_command.github_oauth_start.sync") as start_mock,
            patch(
                "dlt._workspace.cli._runtime_command.github_oauth_complete.sync"
            ) as complete_mock,
        ):
...
def test_runtime_logout_deletes_token_but_keeps_workspace_id(script_runner: ScriptRunner) -> None:
