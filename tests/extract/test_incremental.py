...
def test_composite_primary_key(item_type: TDataItemFormat) -> None:
    ...
    with p.sql_client() as c:
        with c.execute_query(
            "SELECT created_at, isrc, market FROM some_data order by created_at, isrc,"
            " market"
        ) as cur:
            rows = cur.fetchall()
...
def test_remove_incremental_with_incremental_empty() -> None:
    ...
    def some_data_optional(
        last_timestamp: Optional[
            dlt.sources.incremental[float]
        ] = dlt.sources.incremental("item.timestamp"),
    ):
...
def test_incremental_as_transform(item_type: TDataItemFormat) -> None:
    ...
    @dlt.resource
    def some_data():
        last_value: dlt.sources.incremental[float] = (
...
def test_apply_hints_incremental(item_type: TDataItemFormat) -> None:
    ...
    r.apply_hints(
        incremental=dlt.sources.incremental("created_at", last_value_func=min)
    )
    p.extract(r)
    assert "incremental" in r.state
    # max value
...
def test_timezone_naive_datetime(item_type: TDataItemFormat) -> None:
    ...
    def some_data(
        updated_at: dlt.sources.incremental[
            pendulum.DateTime
        ] = dlt.sources.incremental("updated_at", initial_value=pendulum_start_dt),
        max_hours: int = 2,
        tz: str = None,
    ):
        data = [
    ...
    resource.apply_hints(
        incremental=dlt.sources.incremental(
            ...
            end_value=pendulum_start_dt.add(hours=3),
        )
    )
    extract_info = pipeline.extract(resource)
    assert (
    ...
    resource = some_data(max_hours=4).with_name(
        "copy_1"
    )  # also make new resource state
    resource.apply_hints(
        incremental=dlt.sources.incremental("updated_at", initial_value=start_dt)
    )
    ...
    resource.apply_hints(
        incremental=dlt.sources.incremental(
            ...
            end_value=start_dt + timedelta(hours=3),
        )
    )
    data = list(resource)
    if item_type == "json":
    ...
    resource = some_data(max_hours=4, tz="UTC").with_name(
        "copy_3"
    )  # also make new resource state
    resource.apply_hints(
        incremental=dlt.sources.incremental(
...
def test_end_value_initial_value_errors(item_type: TDataItemFormat) -> None:
    ...
    assert (
        "The result of 'custom_last_value([end_value, initial_value])' must equal"
        " 'end_value'"
        in str(ex.value)
    )
...
@pytest.mark.parametrize(
    "deterministic",
    (True, False),
    ids=("deterministic-record", "non-deterministic-record"),
)
def test_unique_values_unordered_rows(
    ...
    assert (
        pipeline.last_trace.last_normalize_info.row_counts["random_ascending_chunks"]
        == 121
    )
    ...
    rows = (
        120
        if primary_key == "updated_at" or (deterministic and primary_key != [])
        else 121
    )
    assert (
        pipeline.last_trace.last_normalize_info.row_counts["random_ascending_chunks"]
