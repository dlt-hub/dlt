...
#     @dlt.transformation
#     def full_t(dataset: dlt.Dataset) -> ir.Table:
...
#             .group_by("city")
#             .aggregate(sum_per_city=ibis._.cost.sum())
#         )
#         return query
...
#     def incremental_t(
#         dataset: dlt.Dataset,
#         load_id=dlt.sources.incremental("_dlt_load_id", on_cursor_value_missing="include"),
#     ) -> ir.Table:
#         customers = dataset.table("customers")
#         incremental_purchases = dataset.table("purchases", incremental=load_id)

#         query = (
#             customers.join(incremental_purchases, customers.id == incremental_purchases.customer_id)
#             .group_by("city")
#             .aggregate(sum_per_city=ibis._.cost.sum())
#         )
...
#     full_results_0 = {row[0]: row[1] for row in t_dataset.table("full_t").to_pyarrow().to_pylist()}
#     incremental_results_0 = {
#         row[0]: row[1] for row in t_dataset.table("incremental_t").to_pyarrow().to_pylist()
#     }

#     # we expect `full` and `incremental` to match on first T run
#     assert full_results_0 == incremental_results_0 == {"kreuzberg": 15, "berlin": 24, "neukölln": 4}
