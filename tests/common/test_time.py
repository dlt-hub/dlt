...
# Legacy test_params for backward compatibility with existing test
test_params = [(params[0], params[1]) for params in datetime_test_params]
...
def test_ensure_pendulum_datetime_non_utc(
    ...
    with local_timezone(local_tz):
        ...
        if dt.tzinfo or expected_non_utc.tzinfo:
            assert dt.tzinfo.utcoffset(dt) == expected_non_utc.tzinfo.utcoffset(expected_non_utc)
        # always pendulum
        assert isinstance(dt, pendulum.DateTime)
...
def test_ensure_pendulum_datetime(
    local_tz: str, date_value: TAnyDateTime, expected: pendulum.DateTime
) -> None:
