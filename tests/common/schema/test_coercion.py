...
def test_coerce_same_type() -> None:
    # same type coercion
    assert utils.coerce_value("double", "double", 8721.1) == 8721.1
    assert utils.coerce_value("bigint", "bigint", 8721) == 8721
    # try various special types
    for k, v in JSON_TYPED_DICT.items():
        typ_ = JSON_TYPED_DICT_TYPES[k]
        assert utils.coerce_value(typ_, typ_, v)


def test_coerce_type_to_text() -> None:
    assert utils.coerce_value("text", "bool", False) == str(False)
    # double into text
    assert utils.coerce_value("text", "double", -1726.1288) == "-1726.1288"
    # bytes to text (base64)
    assert utils.coerce_value("text", "binary", b'binary string') == "YmluYXJ5IHN0cmluZw=="
    # HexBytes to text (hex with prefix)
    assert utils.coerce_value("text", "binary", HexBytes(b'binary string')) == "0x62696e61727920737472696e67"


def test_coerce_type_to_bool() -> None:
    # text into bool
    assert utils.coerce_value("bool", "text", "False") is False
    assert utils.coerce_value("bool", "text", "yes") is True
    assert utils.coerce_value("bool", "text", "no") is False
    # some numeric types
    assert utils.coerce_value("bool", "bigint", 1) is True
    assert utils.coerce_value("bool", "bigint", 0) is False
    assert utils.coerce_value("bool", "decimal", Decimal(1)) is True
    assert utils.coerce_value("bool", "decimal", Decimal(0)) is False

    # no coercions
    with pytest.raises(ValueError):
        utils.coerce_value("bool", "complex", {"a": True})
    with pytest.raises(ValueError):
        utils.coerce_value("bool", "binary", b'True')
    with pytest.raises(ValueError):
        utils.coerce_value("bool", "timestamp", pendulum.now())


def test_coerce_type_to_double() -> None:
    # bigint into double
    assert utils.coerce_value("double", "bigint", 762162) == 762162.0
    # text into double if parsable
    assert utils.coerce_value("double", "text", " -1726.1288 ") == -1726.1288
    # hex text into double
    assert utils.coerce_value("double", "text",  "0xff") == 255.0
    # wei, decimal to double
    assert utils.coerce_value("double", "wei", Wei.from_int256(2137, decimals=2)) == 21.37
    assert utils.coerce_value("double", "decimal", Decimal("-1121.11")) == -1121.11
    # non parsable text
    with pytest.raises(ValueError):
        utils.coerce_value("double", "text", "a912.12")
    # bool does not coerce
    with pytest.raises(ValueError):
        utils.coerce_value("double", "bool", False)


def test_coerce_type_to_bigint() -> None:
    assert utils.coerce_value("bigint", "text", " -1726 ") == -1726
    # for round numerics we can convert
    assert utils.coerce_value("bigint", "double", -762162.0) == -762162
    assert utils.coerce_value("bigint", "decimal", Decimal("1276.0")) == 1276
    assert utils.coerce_value("bigint", "wei", Wei("1276.0")) == 1276

    # raises when not round
    with pytest.raises(ValueError):
        utils.coerce_value("bigint", "double", 762162.1)
    with pytest.raises(ValueError):
        utils.coerce_value("bigint", "decimal", Decimal(912.12))
    with pytest.raises(ValueError):
        utils.coerce_value("bigint", "wei", Wei(912.12))

    # non parsable floats and ints
    with pytest.raises(ValueError):
        utils.coerce_value("bigint", "text", "f912.12")
    with pytest.raises(ValueError):
        utils.coerce_value("bigint", "text", "912.12")
...
def test_coerce_to_numeric(dec_cls: Type[Any], data_type: TDataType) -> None:
    v = utils.coerce_value(data_type, "text", " -1726.839283 ")
    assert type(v) is dec_cls
    assert v == dec_cls("-1726.839283")
    v = utils.coerce_value(data_type, "bigint", -1726)
    assert type(v) is dec_cls
    assert v == dec_cls("-1726")
    # mind that 1276.37 does not have binary representation as used in float
    v = utils.coerce_value(data_type, "double", 1276.37)
    assert type(v) is dec_cls
    assert v.quantize(Decimal("1.00")) == dec_cls("1276.37")

    # wei to decimal and reverse
    v = utils.coerce_value(data_type, "decimal", Decimal("1276.37"))
    assert type(v) is dec_cls
    assert v == dec_cls("1276.37")
    v = utils.coerce_value(data_type, "wei", Wei("1276.37"))
    assert type(v) is dec_cls
    assert v == dec_cls("1276.37")

    # invalid format
    with pytest.raises(ValueError):
        utils.coerce_value(data_type, "text", "p912.12")


def test_coerce_type_from_hex_text() -> None:
    # hex text into various types
    assert utils.coerce_value("wei", "text", " 0xff") == 255
    assert utils.coerce_value("bigint", "text", " 0xff") == 255
    assert utils.coerce_value("decimal", "text", " 0xff") == Decimal(255)
    assert utils.coerce_value("double", "text", " 0xff") == 255.0


def test_coerce_type_to_timestamp() -> None:
    # timestamp cases
    assert utils.coerce_value("timestamp", "text", " 1580405246 ") == pendulum.parse("2020-01-30T17:27:26+00:00")
    # the tenths of microseconds will be ignored
    assert utils.coerce_value("timestamp", "double", 1633344898.7415245) == pendulum.parse("2021-10-04T10:54:58.741524+00:00")
    # if text is ISO string it will be coerced
    assert utils.coerce_value("timestamp", "text", "2022-05-10T03:41:31.466000+00:00") == pendulum.parse("2022-05-10T03:41:31.466000+00:00")
    assert utils.coerce_value("timestamp", "text", "2022-05-10T03:41:31.466+02:00") == pendulum.parse("2022-05-10T01:41:31.466Z")
    assert utils.coerce_value("timestamp", "text", "2022-05-10T03:41:31.466+0200") == pendulum.parse("2022-05-10T01:41:31.466Z")
    # parse almost ISO compliant string
    assert utils.coerce_value("timestamp", "text", "2022-04-26 10:36+02") == pendulum.parse("2022-04-26T10:36:00+02:00")
    assert utils.coerce_value("timestamp", "text", "2022-04-26 10:36") == pendulum.parse("2022-04-26T10:36:00+00:00")
    # parse date string
    assert utils.coerce_value("timestamp", "text", "2021-04-25") == pendulum.parse("2021-04-25", exact=True)

    # fails on "now" - yes pendulum by default parses "now" as .now()
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "now")

    # fails on intervals - pendulum by default parses a string into: datetime, data, time or interval
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "2007-03-01T13:00:00Z/2008-05-11T15:30:00Z")
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "2011--20012")
    ...
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "06/04/22, 11:15PM IST")

    # we do not parse RFC 822, 2822, 850 etc.
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "Wed, 06 Jul 2022 11:58:08 +0200")
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "Tuesday, 13-Sep-22 18:42:31 UTC")

    # time data type not supported yet
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "10:36")

    # non parsable datetime
    with pytest.raises(ValueError):
        utils.coerce_value("timestamp", "text", "x2022-05-10T03:41:31.466000X+00:00")


def test_coerce_type_to_binary() -> None:
    # from hex string
    assert utils.coerce_value("binary", "text", "0x30") == b'0'
    # from base64
    assert utils.coerce_value("binary", "text", "YmluYXJ5IHN0cmluZw==") == b'binary string'
    # int into bytes
    assert utils.coerce_value("binary", "bigint", 15) == b"\x0f"
    # can't into double
    with pytest.raises(ValueError):
        utils.coerce_value("binary", "double", 912.12)
    # can't broken base64
    with pytest.raises(ValueError):
        assert utils.coerce_value("binary", "text", "!YmluYXJ5IHN0cmluZw==")
