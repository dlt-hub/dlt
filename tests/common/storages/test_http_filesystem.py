...
HTTP_BUCKET_URL = "http://localhost:8190"

# expected sizes of the csv sample files, keyed by relative path
CSV_SAMPLE_SIZES = {
...
@pytest.fixture
def http_fs() -> fsspec.AbstractFileSystem:
...
@pytest.mark.serial
def test_glob_http_without_file_info(autoindex_http_server, http_fs) -> None:
    """fsspec scrapes http listings from an html index which reports no size, so files list
    without one instead of failing."""
    items = _by_relative_path(list(glob_files(http_fs, HTTP_BUCKET_URL, "csv/*.csv")))
...
@pytest.mark.serial
def test_glob_http_with_file_info(autoindex_http_server, http_fs) -> None:
    """`fetch_file_info` reads size and mtime per file, which is the only place http reports them."""
    items = _by_relative_path(
        list(glob_files(http_fs, HTTP_BUCKET_URL, "csv/*.csv", fetch_file_info=True))
    )
...
def test_glob_http_recursive(autoindex_http_server, http_fs, fetch_file_info: bool) -> None:
    items = _by_relative_path(
        list(glob_files(http_fs, HTTP_BUCKET_URL, "**/*.csv", fetch_file_info=fetch_file_info))
    )
...
@pytest.mark.serial
@pytest.mark.parametrize("fetch_file_info", (True, False), ids=("with_info", "without_info"))
def test_glob_http_single_file(autoindex_http_server, http_fs, fetch_file_info: bool) -> None:
    """A glob without wildcards resolves through `info` so it reports size either way."""
    items = list(
        glob_files(http_fs, HTTP_BUCKET_URL, "csv/mlb_players.csv", fetch_file_info=fetch_file_info)
    )
...
@pytest.mark.serial
def test_glob_http_modification_date_is_stdlib_datetime(autoindex_http_server, http_fs) -> None:
    import pendulum

    items = list(glob_files(http_fs, HTTP_BUCKET_URL, "csv/*.csv", fetch_file_info=True))

    for item in items:
        assert isinstance(item["modification_date"], datetime)
        assert not isinstance(item["modification_date"], pendulum.DateTime)
        assert item["modification_date"].tzinfo is timezone.utc
