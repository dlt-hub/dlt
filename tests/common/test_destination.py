...
def test_normalize_make_dataset_name() -> None:
    ...
    assert DestinationClientDwhConfiguration(dataset_name="ban_ana_dataset", default_schema_name="default").make_dataset_name(Schema("banana")) == "ban_ana_dataset_banana"
    # without schema name appended
    assert DestinationClientDwhConfiguration(dataset_name="ban_ana_dataset", default_schema_name="default").make_dataset_name(Schema("default")) == "ban_ana_dataset"
    ...
    with pytest.raises(InvalidDatasetName):
        DestinationClientDwhConfiguration(dataset_name="BaNaNa", default_schema_name="default").make_dataset_name(Schema("banana"))

    # empty schemas are invalid
    with pytest.raises(ValueError):
        DestinationClientDwhConfiguration(dataset_name="banana_dataset", default_schema_name=None).make_dataset_name(Schema(None))
    with pytest.raises(ValueError):
        DestinationClientDwhConfiguration(dataset_name="banana_dataset", default_schema_name="").make_dataset_name(Schema(""))

    # empty dataset names are invalid
    with pytest.raises(ValueError):
        DestinationClientDwhConfiguration(dataset_name="", default_schema_name="ban_schema").make_dataset_name(Schema("schema_ana"))
    with pytest.raises(ValueError):
        DestinationClientDwhConfiguration(dataset_name=None, default_schema_name="BAN_ANA").make_dataset_name(Schema("BAN_ANA"))

    # now mock the schema name to make sure that it is normalized
    schema = Schema("barbapapa")
    schema._schema_name = "BarbaPapa"
    assert DestinationClientDwhConfiguration(dataset_name="set", default_schema_name="default").make_dataset_name(schema) == "set_barba_papa"


def test_normalize_make_dataset_name_none_default_schema() -> None:
    # if default schema is None, suffix is not added
    assert DestinationClientDwhConfiguration(dataset_name="ban_ana_dataset", default_schema_name=None).make_dataset_name(Schema("default")) == "ban_ana_dataset"
