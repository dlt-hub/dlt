...
def test_child_table_linking(schema: Schema) -> None:
    ...
    assert all(e[1]["_root_hash"] == row_id for e in rows if e[0][0] != "table")
    # all child entries must have _record_hash
    assert all("_record_hash" in e[1] for e in rows if e[0][0] != "table")
    # all child entries must have parent hash and pos
    assert all("_parent_hash" in e[1] for e in rows if e[0][0] != "table")
    assert all("_pos" in e[1] for e in rows if e[0][0] != "table")
...
def test_child_row_deterministic_hash(schema: Schema) -> None:
    ...
    el_f = next(t[1] for t in rows if t[0][0] == "table__f" and t[1]["_pos"] == 0)
    f_lo_p2 = next(t[1] for t in rows if t[0][0] == "table__f__lo" and t[1]["_pos"] == 2)
...
def test_extract_with_table_name_meta() -> None:
    ...
    assert "_record_hash" in normalized_row
    assert normalized_row["_load_id"] == "load_id"
...
def test_parse_with_primary_key() -> None:
    ...
    f_wo_id = next(t[1] for t in rows if t[0][0] == "discord__w_id__wo_id" and t[1]["_pos"] == 2)
