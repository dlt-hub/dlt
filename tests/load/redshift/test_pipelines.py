...
#     p.create_pipeline(FAKE_CREDENTIALS, working_dir=os.path.join(TEST_STORAGE_ROOT, FAKE_CREDENTIALS.DEFAULT_DATASET), schema=Schema("table"))
#     p.extract(iter(["a", "b", "c"]), table_name="test")
#     p.normalize()
...
#     p.create_pipeline(FAKE_CREDENTIALS, working_dir=os.path.join(TEST_STORAGE_ROOT, FAKE_CREDENTIALS.DEFAULT_DATASET), schema=Schema("matrix"))
#     assert len(p.list_normalized_loads()) == 0
