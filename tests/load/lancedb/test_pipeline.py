...
def test_merge_github_nested(destination_config: DestinationTestConfiguration) -> None:
    pipe = destination_config.setup_pipeline("github1", dataset_name="github1", dev_mode=True)
...
def test_bring_your_own_vector(destination_config: DestinationTestConfiguration) -> None:
    ...
    with pipeline.destination_client() as client:
        ...
        assert len(read_arrow_table(tbl).to_pandas()) == num_rows
...
def test_empty_dataset_allowed(destination_config: DestinationTestConfiguration) -> None:
    # `lance` keeps `dataset_name` optional, so it is not autogenerated and tables go to the root
    # namespace. `lancedb` needs one because a dataset is a database there. embeddings are tested
    # elsewhere, so this stays plain text.
    destination: Any = destination_config.destination_factory()
    pipe = dlt.pipeline(destination=destination, dev_mode=True)

    assert pipe.dataset_name is None
    info = pipe.run(["context", "created", "not a stop word"], table_name="content")
    ...
    if supports_sql_reads(destination_config):
...
def test_dataset_sees_schema_evolution_with_always_refresh(
    destination_config: DestinationTestConfiguration,
) -> None:
    """With `always_refresh_views` enabled, a dataset opened with a persisted sql client also sees
    schema changes (new columns) written after it was opened."""
    pipeline = destination_config.setup_pipeline(
        "lance_open_conn_refresh_" + uniq_id(), dev_mode=True
    )
    pipeline.destination.config_params["always_refresh_views"] = True
    ...
    with pipeline.dataset() as ds_:
        ...
        # append a row AND evolve the schema (new `extra` column) while the connection stays open
        info = pipeline.run([{"id": 2, "value": "b", "extra": 99}], table_name="items")
        assert_load_info(info)

        # both the new row and the new column are visible through the refreshed view
        result = ds_("SELECT * FROM items").arrow()
...
def test_semantic_query(destination_config: DestinationTestConfiguration) -> None:
    ...
    with pipeline.destination_client() as client:
        ...
        # a managed table does not infer the vector column from its embedding metadata
        results = (
            tbl.search("puppy", query_type="vector", vector_column_name=vector_column_name(client))
            .select(["text"])
            .to_list()
        )
...
def test_dataset_is_a_database_recorded_by_a_sentinel(
    destination_config: DestinationTestConfiguration,
) -> None:
    """A database holding no tables looks like one that never existed, so a sentinel records it."""
    pipeline = dlt.pipeline(
        pipeline_name="lancedb_sentinel_" + uniq_id(),
        destination=destination_config.destination_factory(),
        dataset_name="sentinel_" + uniq_id(),
    )
    with pipeline.destination_client() as client:
        client = cast("LanceDBClient", client)
        sentinel = client.config.dataset_sentinel_namespace_name
        assert not client.is_storage_initialized()

        client.initialize_storage()
        assert client.is_storage_initialized()
        assert client.list_namespace_names() == [sentinel]
        # the dataset is the database, and its tables live in the root namespace
        assert client.dataset_name == pipeline.dataset_name
        assert client.make_namespace_path() == []

        assert_load_info(pipeline.run([{"id": 1}], table_name="items"))
        assert "items" in client.list_table_names()

        client.drop_storage()
        assert not client.is_storage_initialized()
        assert client.list_namespace_names() == []
        assert client.list_table_names() == []

        # the emptied database can be used again
        client.initialize_storage()
        assert client.is_storage_initialized()
        client.drop_storage()
...
def test_drop_storage_keeps_foreign_dataset(
    ...
    pipeline = dlt.pipeline(
        pipeline_name="lancedb_pinned_drop_" + uniq_id(),
        destination=destination_config.destination_factory(),
    )
    assert_load_info(pipeline.run([{"id": 1}], table_name="items"))
    ...
    with pipeline.destination_client() as client:
        client = cast("LanceDBClient", client)
        assert client.dataset_name == pinned_database
        ...
        try:
            ...
            remaining = client.list_table_names()
            assert foreign_table_name in remaining
            assert "items" not in remaining
            assert "_dlt_loads" not in remaining
        finally:
