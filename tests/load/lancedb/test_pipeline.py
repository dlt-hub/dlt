...
def test_empty_dataset_allowed(destination_config: DestinationTestConfiguration) -> None:
    ...
    # both lance (root namespace) and lancedb (no table prefix) allow this. embeddings are tested
    # elsewhere, so this stays plain text.
    destination: Any = (
        destination_config.destination_factory()
        if destination_config.destination_type == "lance"  # lance needs catalog/bucket config
