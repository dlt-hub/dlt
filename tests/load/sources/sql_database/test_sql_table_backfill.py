...
#test_load_sql_table_resource_incremental_end_value
...
def test_load_sql_table_resource_incremental_end_value(
    ...
    with engine.connect() as conn:
        result = conn.exec_driver_sql("SELECT MIN(id), MAX(id) FROM {}.chat_message".format(sql_source_db.schema))
        min_id, max_id = result.fetchone()

        # Read all IDs from source table for verification
        result = conn.exec_driver_sql("SELECT id FROM {}.chat_message ORDER BY id".format(sql_source_db.schema))
        source_ids = [row[0] for row in result.fetchall()]

    # create 8 equal ranges from min and max id + 1
    step = (max_id - min_id + 1) // 8
    ranges = [
        {"min_id": min_id + i * step, "max_id": min_id + (i + 1) * step}
        for i in range(8)
    ]
    ...
    all_tables = [
        ...
        ).with_name(f"chat_message_{i}").apply_hints(table_name="chat_message").parallelize()
        for i, r in enumerate(ranges)
    ]
    
    pipeline = dlt.pipeline("test_load_sql_table_resource_incremental_end_value", destination="duckdb")
    ...
    assert load_data_table_counts(pipeline)["chat_message"] == sql_source_db.table_infos["chat_message"]["row_count"] - 1
...
def test_load_sql_table_split_loading(
    ...
    # The total should match the number of rows in the chat_message table
    assert load_data_table_counts(pipeline)["chat_message"] == sql_source_db.table_infos["chat_message"]["row_count"]
