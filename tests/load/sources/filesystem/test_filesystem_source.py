...
def test_incremental_load(
    bucket_url: str, destination_config: DestinationTestConfiguration, incremental_method: str, row_order: TSortOrder
) -> None:
    @dlt.transformer
    def bypass(items) -> str:
        return items
    ...
    def _get_files() -> filesystem:
        incremental_ = dlt.sources.incremental("modification_date", row_order=row_order)
...
def test_incremental_order(
    bucket_url: str, incremental_method: str, row_order: TSortOrder
) -> None:

    pipeline = dlt.pipeline("test_incremental_order", destination="duckdb", dev_mode=True)

    def _get_files() -> dlt.sources.DltResource:
        incremental_ = dlt.sources.incremental("file_name", row_order=row_order)
        if incremental_method == "signature":
