...
def test_clickhouse_atomic_swap_replace(
    destination_config: DestinationTestConfiguration,
    mocker: MockerFixture,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    clone_spy = mocker.spy(SqlStagingReplaceFollowupJob, "_generate_clone_sql")
    insert_spy = mocker.spy(SqlStagingFollowupJob, "_generate_insert_sql")
    swap_spy = mocker.spy(ClickHouseStagingReplaceJob, "generate_sql")

    pipeline = destination_config.setup_pipeline("clickhouse_atomic_swap_test", dev_mode=True)
    ...
    assert clone_spy.call_count == 0
    assert insert_spy.call_count == 0
    for sql_stmt in swap_spy.spy_return:
        assert "EXCHANGE TABLES" in sql_stmt or "TRUNCATE TABLE" in sql_stmt
    assert any("EXCHANGE TABLES" in sql for sql in swap_spy.spy_return)
    ...
    @dlt.resource(name="items", write_disposition="replace", primary_key="id")
    def load_items_with_new_column(offset):
        for i in range(offset, offset + 3):
            yield {
                "id": i,
                "name": f"item {i}",
    ...
    swap_spy.reset_mock()
    info = pipeline.run(load_items_with_new_column(200), **destination_config.run_kwargs)
    assert_load_info(info)
    assert swap_spy.call_count == 1
    assert "category" in pipeline.default_schema.get_table("items")["columns"]
    table_dicts = load_tables_to_dicts(pipeline, "items")
    assert {int(r["id"]) for r in table_dicts["items"]} == {200, 201, 202}
    assert {r["category"] for r in table_dicts["items"]} == {"cat 200", "cat 201", "cat 202"}

    # fourth load: empty resource clears all tables
    @dlt.resource(name="items", write_disposition="replace", primary_key="id")
    def load_items_empty():
        if False:
            yield
