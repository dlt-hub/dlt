...
def test_regular_keys() -> None:
    ...
    c = resolve_configuration(GcpServiceAccountCredentials(), sections=(known_sections.DESTINATION, "bigquery"))
    ...
    assert provider.get_value("secret_key", AnyType, "pipeline", "api") == ("ABCDE", "pipeline-api-secret_key")
    assert provider.get_value("credentials", AnyType, "pipeline") == ({"project_id": "mock-credentials-pipeline"}, "pipeline-credentials")
    ...
    assert provider.get_value("only_pipeline_top", AnyType, "pipeline", "sources") == ("TOP", "pipeline-sources-only_pipeline_top")
    # get element unique to sources
    assert provider.get_value("all_sources_present", AnyType, None, "sources") == (True, "sources-all_sources_present")
    ...
    # non secrets allowed
    assert provider.get_value("secret", str, "pipeline") == ("THIS IS SECRET VALUE", "pipeline-secret")
