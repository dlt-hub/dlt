...
from dlt.dbt_runner import run_dbt_in_venv, run_dbt
...
def run_dbt_f(request: Any) -> AnyFun:
    if request.param is None:
        yield run_dbt
    else:
        with Venv.create(tempfile.mkdtemp(), [os.getcwd()] + request.param) as venv:
            yield partial(run_dbt_in_venv, venv)
    # yield partial(run_dbt_in_venv, Venv.restore_current())
...
def test_run_jaffle_from_repo(client: PostgresClient, test_storage: FileStorage, run_dbt_f: AnyFun) -> None:
    test_storage.create_folder("jaffle")
    results = run_dbt_f(client.config, test_storage.make_full_path("jaffle"), JAFFLE_SHOP_REPO, ["--fail-fast", "--full-refresh"])
    print(results)
    assert_jaffle_completed(test_storage, results)
    # incremental run
    results = run_dbt_f(client.config, test_storage.make_full_path("jaffle"), JAFFLE_SHOP_REPO)
    print(results)


def test_run_jaffle_from_folder_incremental(client: PostgresClient, test_storage: FileStorage, run_dbt_f: AnyFun) -> None:
    ...
    results = run_dbt_f(client.config, repo_path, repo_path, package_run_params=None)
...
def test_run_jaffle_fail_prerequisites(client: PostgresClient, test_storage: FileStorage, run_dbt_f: AnyFun) -> None:
    test_storage.create_folder("jaffle")
    # we run all the tests before tables are materialized
    with pytest.raises(PrerequisitesException) as pr_exc:
        run_dbt_f(client.config, test_storage.make_full_path("jaffle"), JAFFLE_SHOP_REPO, ["--fail-fast", "--full-refresh"], "*")
...
def test_run_jaffle_invalid_run_args(client: PostgresClient, test_storage: FileStorage, run_dbt_f: AnyFun) -> None:
    test_storage.create_folder("jaffle")
    # we run all the tests before tables are materialized
    with pytest.raises(DBTProcessingError) as pr_exc:
        run_dbt_f(client.config, test_storage.make_full_path("jaffle"), JAFFLE_SHOP_REPO, ["--wrong_flag"])
    assert isinstance(pr_exc.value.dbt_results, SystemExit)


def test_run_jaffle_failed_run(client: PostgresClient, test_storage: FileStorage, run_dbt_f: AnyFun) -> None:
    repo_path = clone_jaffle_repo(test_storage)
    # copy model with error into package to force run error in model
    shutil.copy("./tests/dbt_runner/cases/jaffle_customers_with_error.sql", os.path.join(repo_path, "models", "customers.sql"))
    with pytest.raises(DBTProcessingError) as pr_exc:
        run_dbt_f(client.config, repo_path, repo_path, package_run_params=None)
