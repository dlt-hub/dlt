...
def test_setup_dbt_runner() -> None:
    runner = setup_rasa_runner("redshift", "carbon_bot_3", override_values={
    ...
    assert runner._get_package_vars() == {"source_dataset_name": "carbon_bot_3", "add_var_name": "add_var_value"}
...
def test_initialize_package_wrong_key() -> None:
    runner = setup_rasa_runner("redshift", override_values={
        # private repo
...
def test_dbt_run_error_via_additional_vars(destination_info: DBTDestinationInfo) -> None:
    ...
    with pytest.raises(DBTProcessingError) as dbt_err:
        runner.run_all(
            destination_dataset_name=DESTINATION_DATASET_NAME,
            run_params=["--fail-fast", "--full-refresh"],
            additional_vars={"user_id": "metadata__user_id", "external_session_id": "metadata__sess_id"},
            source_tests_selector="tag:prerequisites"
        )
