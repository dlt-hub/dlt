...
def test_coerce_new_null_value(item_normalizer: JsonLItemsNormalizer) -> None:
    # case 1: no table at all — null column tracked, no schema update
    row = {"timestamp": None}
    new_row, new_table = item_normalizer._coerce_row("event_user", None, row)
    assert "timestamp" not in new_row
    assert new_table is None
    assert item_normalizer._null_only_columns["event_user"] == {"timestamp"}

    # case 2: table exists with a bound column — null column tracked alongside real column
    _, new_table = item_normalizer._coerce_row("event_user", None, {"value": "0xFF", "x": None})
    assert "value" in new_table["columns"]
    assert "x" not in new_table["columns"]
    assert item_normalizer._null_only_columns["event_user"] == {"timestamp", "x"}
...
def test_coerce_null_value_over_existing_predefined(item_normalizer: JsonLItemsNormalizer) -> None:
    ...
    item_normalizer.schema.update_table(new_table)
    # null on predefined incomplete col: tracked in null_only_columns, no schema update for it
    row = {"timestamp": 82178.1298812, "null_col": None}
    _, new_table = item_normalizer._coerce_row("event_user_2", None, row)
    assert "timestamp" in new_table["columns"]
    assert "null_col" not in new_table["columns"]
    assert "null_col" in item_normalizer._null_only_columns["event_user_2"]
    item_normalizer.schema.update_table(new_table)
    # same row again
    _, new_table = item_normalizer._coerce_row("event_user_2", None, row)
    # do not generate table update on existing column
...
def test_coerce_null_value_in_nested_table(
    ...
    # null values for columns already created as nested tables don't create schema updates
    schema_update = _normalize_items_chunk(
        "nested",
        [
            {
...
def test_coerce_null_value_as_compound_columns(
    ...
    """After a dict value creates compound columns, null for the same key
    produces no schema update, and a simple value creates a new column."""
    schema_update = item_normalizer._normalize_chunk(
        root_table_name="nested",
        items=[{"id": "1", "a": {"b": 1, "c": 2}}],
        may_have_pua=False,
        skip_write=True,
    )
    assert "nested" in schema_update
    ...
    # null for the same key: no schema update
    schema_update = item_normalizer._normalize_chunk(
        root_table_name="nested",
        items=[{"id": "1", "a": None}],
        may_have_pua=False,
    ...
    # simple value for the same key: creates a new column
    schema_update = item_normalizer._normalize_chunk(
        root_table_name="nested",
        items=[{"id": "1", "a": "col"}],
        may_have_pua=False,
    ...
    assert "a" in schema_update["nested"][0]["columns"]
...
def test_variant_column_has_description(item_normalizer: JsonLItemsNormalizer) -> None:
    """Test that variant columns get a default description when is_variant is True."""
    # Create a column with variant=True using _infer_column
    variant_col = item_normalizer._infer_column("test_col", "some_value", is_variant=True)

    assert variant_col is not None
    assert variant_col["variant"] is True
    assert "description" in variant_col
    assert variant_col["description"] == "Variant of column test_col generated by dlt"


def test_non_variant_column_has_no_default_description(
    item_normalizer: JsonLItemsNormalizer,
) -> None:
    """Test that non-variant columns do not get the variant description."""
    # Create a column with variant=False (default)
    normal_col = item_normalizer._infer_column("test_col", "some_value", is_variant=False)

    assert normal_col is not None
    assert "variant" not in normal_col
    assert "description" not in normal_col


def test_variant_column_description_via_coerce_row(
    item_normalizer: JsonLItemsNormalizer,
) -> None:
    """Test that variant columns created via _coerce_row have the description."""
    # First create a column with double type
    row_1 = {"confidence": 0.1}
    _, new_table = item_normalizer._coerce_row("event_user", None, row_1)
    item_normalizer.schema.update_table(new_table)

    # Now send a value that cannot be coerced to double, triggering variant creation
    row_2 = {"confidence": "NOT_A_NUMBER"}
    _, new_table = item_normalizer._coerce_row("event_user", None, row_2)

    new_columns = list(new_table["columns"].values())
    variant_col = new_columns[0]

    assert variant_col["name"] == "confidence__v_text"
    assert variant_col["variant"] is True
    assert "description" in variant_col
    assert variant_col["description"] == "Variant of column confidence__v_text generated by dlt"


def test_variant_column_description_with_bool_variant(
    item_normalizer: JsonLItemsNormalizer,
) -> None:
    """Test that bool variant columns have the description."""
    # Create a double column first
    row_1 = {"value": 123.45}
    _, new_table = item_normalizer._coerce_row("test_table", None, row_1)
    item_normalizer.schema.update_table(new_table)

    # Send a bool value to trigger variant creation
    row_2 = {"value": False}
    _, new_table = item_normalizer._coerce_row("test_table", None, row_2)

    variant_col = new_table["columns"]["value__v_bool"]

    assert variant_col["variant"] is True
    assert variant_col["description"] == "Variant of column value__v_bool generated by dlt"
