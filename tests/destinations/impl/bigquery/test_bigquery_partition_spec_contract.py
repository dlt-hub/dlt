...
class TestBigQueryPartitionStrictHandling:
    ...
    def test_reconstruct_partition_spec_with_invalid_type_raises_error(
    ...
    def test_reconstruct_partition_spec_with_missing_type_key_raises_error(
        self, client: BigQueryClient
    ):
        """Test that _reconstruct_partition_spec raises ValueError if '_dlt_partition_spec_type' is missing."""
        partition_dict = {"column_name": "created_at", "granularity": "DAY"}
        with pytest.raises(
            ValueError, match=r"Missing '_dlt_partition_spec_type' in partition hint dict.*"
        ):
    ...
    def test_bigquery_partition_clause_with_invalid_type_raises_error(self, client: BigQueryClient):
        ...
        with pytest.raises(
            ValueError,
            match=(
                r"_bigquery_partition_clause only accepts BigQueryPartitionSpec instances, got"
                r" str.*"
            ),
        ):
    ...
    def test_bigquery_partition_clause_with_valid_spec_succeeds(self, client: BigQueryClient):
        ...
        spec = BigQueryTimestampTruncPartition(column_name="created_at", granularity="DAY")
