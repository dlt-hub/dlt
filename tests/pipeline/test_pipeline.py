...
@pytest.mark.parametrize("mark_main_resource", (True, False))
def test_nested_inserts_correct_target(mark_main_resource: bool) -> None:
    @dlt.resource()
    def my_resource():
        yield [
            {
    ...
    @dlt.transformer(data_from=my_resource)
    def things(
        ...
        for my_resource in my_resources:
            fields: List[TDataItem] = my_resource.pop("fields")
            if mark_main_resource:
                yield dlt.mark.with_hints(
                    item=my_resource,
