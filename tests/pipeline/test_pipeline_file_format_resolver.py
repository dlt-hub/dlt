...
def test_file_format_resolution() -> None:
    ...
    with pytest.raises(DestinationIncompatibleLoaderFileFormatException):
        assert p._resolve_loader_file_format("some", "some", destcp, None, "tsv")  # type: ignore[arg-type]
    ...
    # check invalid input
    with pytest.raises(DestinationIncompatibleLoaderFileFormatException):
        p._resolve_loader_file_format("some", "some", destcp, stagecp, "tsv")  # type: ignore[arg-type]
