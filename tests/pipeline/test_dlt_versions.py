...
def test_pipeline_with_versions(test_storage: FileStorage) -> None:
        ...
        with custom_environ({"DLT_DATA_DIR": get_dlt_data_dir()}):
            ...
            with custom_environ({"DESTINATION__DUCKDB__CREDENTIALS": "duckdb:///test_github_3.duckdb"}):
                ...
                with Venv.create(tempfile.mkdtemp(), ["dlt[duckdb]==0.3.0"]) as venv:
                    ...
                    github_schema: TStoredSchema = json.loads(test_storage.load(".dlt/pipelines/dlt_github_pipeline/schemas/github.schema.json"))
                    ...
                    duckdb_cfg = resolve_configuration(DuckDbClientConfiguration(dataset_name="github_3"), sections=("destination", "duckdb"))
                    with DuckDbSqlClient("github_3", duckdb_cfg.credentials) as client:
                    ...
                    # hash hash in schema
                    github_schema = json.loads(test_storage.load(".dlt/pipelines/dlt_github_pipeline/schemas/github.schema.json"))
                    ...
                    pipeline = dlt.attach("dlt_github_pipeline", credentials=duckdb_cfg.credentials)
                    pipeline = pipeline.drop()
                    # print(pipeline.working_dir)
                    assert pipeline.dataset_name == "github_3"
